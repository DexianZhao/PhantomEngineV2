...
PHANTOM_API CLuaFileMgr		g_luamanager;
...
BOOL		CLuaFile::LoadText(const char* szText, int len)//从文本载入
{
	...
	char ret[10240];
	TypeArray<char> buffer2;
	int lastLen = len + 10;
	buffer2.resize(len + 10);
...
TypeArray<RandItem>	g_randomTables;
VOID	WINAPI	Global_random_init(int number)
{
	g_randomTables.resize(number);
...
{
	TypeArray<RandItem> randoms = g_randomTables;
	int maxnum = 0;
...
/*API*/void			WINAPI	Global_net_init/*全局_net初始化*/(int dwGUID)
{
	...
	g_socketWrite.buffers.resize(SOCKET_PACKAGE_MAX);
