...
namespace Phantom{
	...
	{CPUTime(Font);
		...
		m_tempBuffer.resize(len * 2, false);
		if(g_manager.GetIsUtf8())
		{
			...
			m_tempBuffer.resize(index);
		}
		else
		{
			int l = Utf8ToW(szUTF8, m_tempBuffer.GetPtr(), len * 2);
			m_tempBuffer.resize(l);
		}
	...
	PHANTOM_API BOOL	ClipRect(Rect& dest, Rect& tex, const Rect& clip)
	{
