...
/*API*/VOID		WINAPI	UIDialog_Setrect/*界面对话框_设置对话框位置*/(IDialogPtr* _self/*接口*/, const Rect* _val/*矩形*/)
{
	...
	d->SetSize(_val->Width(), _val->Height());
}
...
/*API*/VOID		WINAPI	UIDialog_create/*界面对话框_创建控件*/(IDialogPtr* _self/*接口*/, UIControlType type/*界面类型*/,  const Rect* rc/*创建区域*/,  IControlPtr* _return/*ret*/)
{
	...
	case UIControlType_BUTTON:
		c = d->AddButton(true, "", rc->left, rc->top, rc->Width(), rc->Height());
		break;
	case UIControlType_CHECKBOX:
		c = d->AddCheckBox(true, "", rc->left, rc->top, rc->Width(), rc->Height());
		break;
	case UIControlType_COMBOBOX:
		c = d->AddComboBox(true, rc->left, rc->top, rc->Width(), rc->Height());
		break;
	case UIControlType_SLIDER:
		c = d->AddSlider(true, rc->left, rc->top, rc->Width(), rc->Height());
		break;
	case UIControlType_LISTBOX:
		c = d->AddListBox(true, rc->left, rc->top, rc->Width(), rc->Height());
		break;
	case UIControlType_SCROLLBAR:
		c = d->AddScrollBar(true, rc->left, rc->top, rc->Width(), rc->Height());
		break;
	case UIControlType_PROGRESS:
		c = d->AddProgress(true, rc->left, rc->top, rc->Width(), rc->Height());
		break;
	case UIControlType_STATIC:
		c = d->AddStatic(true, "", rc->left, rc->top, rc->Width(), rc->Height());
		break;
	case UIControlType_VIEW_FORMAT_TEXT:
		c = d->AddFormatStatic(true, "", rc->left, rc->top, rc->Width(), rc->Height());
		break;
	case UIControlType_LISTCTRL3D:
		c = d->AddListCtrl(true, rc->left, rc->top, rc->Width(), rc->Height());
		break;
	case UIControlType_ui2DEffect:
		c = d->AddEffect(true, rc->left, rc->top, rc->Width(), rc->Height());
		break;
	}
	if(!c)
		return;
