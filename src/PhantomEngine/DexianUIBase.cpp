...
namespace Phantom {
	...
	{
		DexianTexture* Tex = DexianManager::GetInstance().NewTexture(DexianManager::GetInstance().AddTextureFile(File), TextureFilter_Point, dir);
		if (Tex) {
	...
	//}
	DexianDrawBase::DexianDrawBase(DexianShape* Shape)
	{
		ModeX = AlignLeft;
		ModeY = AlignTop;
		Offset.Set(0, 0);
	...
	void			DexianDrawRect::Render(const RectF& WindowRect)
	{
		...
		g_manager.RenderRect2D(&Buffer, r, color);// .DrawRect2D(r, color, true, false);
	}
	...
	DexianDrawTexture::DexianDrawTexture(DexianShape* Shape):DexianDrawRect(Shape)
	{
		FrameMode = 0;
	...
	void			DexianDrawTexture::Render(const RectF& WindowRect)
	{
		RectF r;
		if (IsCenterMode) {
		...
		if (node)
			g_manager.RenderTexture(&Buffer, node, r, Curr->TextureRegion, color);// .DrawRect2D(r, color, true, false);
		else {
			g_manager.RenderRect2D(&Buffer, r, color);
		}
	}
	...
	void			DexianDrawCircle::Render(const RectF& WindowRect)
	{
		...
		g_manager.RenderCircle2D(&Buffer, C, Radius, Color, Smooth);// .DrawCircle(C, Radius, Color, Smooth, true);
	}
	...
	void			DexianDrawRoundRect::Render(const RectF& WindowRect)
	{
		Rect r = GetRenderRect(WindowRect);
		if (!(r == Last)) {
			Buffer.Valid = false;
		...
		g_manager.RenderRoundRect2D(&Buffer, r, Round, RoundMode, &color);
	...
	DexianDrawEventPoint::DexianDrawEventPoint(DexianViewerPlace* _Place, DexianShape* Shape)
	...
	void			DexianDrawEventPoint::Render(const RectF& WindowRect)
	{
		Rect r = GetRenderRect(WindowRect);
		Rect PR = r;
		if (this->ModeX == AlignRight)
		...
		if (node)
			g_manager.RenderTexture(&Buffer, node, r, E->TextureRegion, color);// .DrawRect2D(r, color, true, false);
		else {
			g_manager.RenderRect2D(&Buffer, r, color);
		}
	}
	...
	DexianDrawText::DexianDrawText(DexianShape* Shape)
	...
	void			DexianDrawText::Render(const RectF& WindowRect)
	{
		if (TextPtr) {
			Rect r = GetRenderRect(WindowRect);
			...
			g_manager.RenderText2D(&Buffer, TextPtr, r, color);
		}
	}
	...
	DexianDrawEditText::DexianDrawEditText(DexianShape* Shape)
		:DexianDrawRect(Shape)
	{
	...
	void			DexianDrawEditText::Render(const RectF& WindowRect)
	{
		if (TextPtr) {
			Rect r = GetRenderRect(WindowRect);
			r.right += 20;
			...
			g_manager.RenderRect2D(&RectBuffer, border, color4(1, 1, 1, 1), false);
			g_manager.RenderText2D(&Buffer, TextPtr, r, color);
		}
	}
	...
	void			DexianDrawCurveLine::Render(const Pixel& from, const Pixel& to)
	{
	...
	bool			DexianDrawCurveLine::OnRender(float fElapsedTime)
	{
	...
	void	DexianViewerPlace::OnRender(float fElapsedTime)
	{
		if (Adding) {
			AddNew->Render(AddingPoint[0], AddingPoint[1]);
		}
		for (int i = 0; i < Lines.size(); i++) {
			if (!Lines[i]->OnRender(fElapsedTime)) {
				delete Lines[i];
				i--;
			}
		}
	...
	BOOL 	DexianControl::IsPtIn(PixelF pt)
	{
		...
		if (!bIsPickPixel)
		{
			//DexianShape* S = DexianManager::GetInstance().GetShape(Back.Shape);
			//if (S)
			//{
	...
	DexianButton*			DexianControl::AddButton(const RectF& clientRect, ButtonClick CB)
	{
	...
	DexianStatic*			DexianControl::AddStatic(const RectF& clientRect)
	{
	...
	DexianShape*			DexianControl::AddShape(const RectF& clientRect)
	{
		DexianShape* dg = new DexianShape(this);
		dg->SetClientRect(clientRect);
		Childs.push_back(dg);
		return dg;
	}
	DexianListBox*			DexianControl::AddListBox(const RectF& clientRect, ListBoxSelected Callback)
	{
	...
	void		DexianControl::Render(float fElapsedTime)
	{
		OnRender(fElapsedTime);
		for (int i = (int)Childs.size() - 1; i >= 0; i--) {
			if (Childs[i]->Display)
				Childs[i]->Render(fElapsedTime);
		}
	}
	...
	void					DexianControl::RenderElement(DexianElement* Element)
	{
		if (!Element)
			return;
	...
	DexianShape::DexianShape(DexianControl* Parent)
		:DexianControl(Parent)
	{
	...
	void	DexianShape::Remove(DexianDrawBase* b)
	{
		for (int i = 0; i < Draws.size(); i++) {
				break;
			if (Draws[i] == b) {
	...
	VOID	DexianShape::FrameMove(double fTime, float fElapsedTime)
	{
		if (RequestDrawSort) {
			RequestDrawSort = false;
	...
	BOOL 	DexianShape::IsPtIn(PixelF pt)
	{
		CPUTime(DexianControl);
		for (int i = 0; i < Draws.size(); i++) {
	...
	char 	DexianShape::OnMouseMessage(unsigned int uMsg, PixelF pos, unsigned int touchIndex)
	{
		if (!ParentPtr)return 0;
		switch (uMsg)
		{
	...
	DexianDrawRect*			DexianShape::AddRect(AlignModeX ModeX, AlignModeY ModeY, const RectF& rc, const color4& c)
	{
	...
	DexianDrawTexture*		DexianShape::AddTexture(AlignModeX ModeX, AlignModeY ModeY, const RectF& rc, const color4& c, const char* File, const RectF* Region, BaseDirectory* dir)
	{
	...
	DexianDrawRoundRect*	DexianShape::AddRoundRect(AlignModeX ModeX, AlignModeY ModeY, const RectF& rc, const color4& c, float fRoundRadius, int nRoundMode)
	{
	...
	DexianDrawText*			DexianShape::AddText(AlignModeX ModeX, AlignModeY ModeY, const char* Text, const color4& c, int FontSize, const char* szFontName, int FontStyle)
	{
	...
	DexianDrawEditText*		DexianShape::AddEditableText(AlignModeX ModeX, AlignModeY ModeY, const RectF& rc, const char* Text, const color4& c, int FontSize, const char* szFontName, int FontStyle)
	{
	...
	DexianDrawCircle*		DexianShape::AddCircle(AlignModeX ModeX, AlignModeY ModeY, const PixelF& center, float Radius, const color4& c, int Smooth)
	{
	...
	DexianDrawEventPoint*			DexianShape::AddEventPoint(AlignModeX ModeX, AlignModeY ModeY, DexianViewerPlace* Place, const RectF& rc, const color4& c, const char* File, const RectF* Region, const char* FullFile, const RectF* FullRegion, BaseDirectory* dir)
	{
		if (!Place)
			return 0;
	...
	void			DexianShape::OnRender(float fElapsedTime)
	{
		...
		const char* IconFile, const char* FullIconFile, BaseDirectory* TextureDir)
	{
		...
		for (int i = 0; i < Events.size(); i++) {
			...
			if (Event.Name.t[0]) {
				...
				Text = this->AddText(isLeft ? AlignLeft : AlignRight, AlignTop, Event.Name, color4(1, 1, 1, 1),
			...
			if (ST) {
				...
				if (isLeft && ST->Limit != Edit_None) {
					DexianDrawEditText* Text = this->AddEditableText(isLeft ? AlignLeft : AlignRight, AlignTop, Rect(0, 0, 60, 26), Event.Name, color4(1, 1, 1, 1),
			...
			DexianDrawEventPoint* EventArrow = AddEventPoint(isLeft ? AlignLeft : AlignRight, AlignTop, Place, RectF(0, 0, IconSize, IconSize), 
	...
	{
		if (Viewer.Name.t[0]) {
			Title = this->AddText(AlignLeftRight, AlignTop, Viewer.Name, color4(1, 1, 1, 1),
		...
		RectTitle = this->AddRoundRect(AlignLeft, AlignTop, RectF(0, 0, 15, 15), color4(0.01f, 0.01f, 0.01f, 0.2), 20.0f, 0xff);
		RectTitle->SetLayer(-2);
		RectBody = this->AddRoundRect(AlignLeft, AlignTop, RectF(0, 0, 15, 15), color4(1, 0, 0, 0.5), 20.0f, 1 | 2);
		RectBody->Shadow = false;
		RectBody->SetLayer(-1);
		AddTexture(AlignLeft, AlignTop, RectF(5, -5, 48, 48), color4(1, 1, 1, 1), "EventIcon.png", 0, TextureDir);
	...
	DexianListLine::DexianListLine(DexianControl* dlg)
		:DexianShape(dlg)
	{
		Icon = 0;
		Text = 0;
	...
	void	DexianListLine::SetupLine(const char* szText, const char* IconFile, BaseDirectory* TextureDir)
	{
		if(IconFile)
			AddTexture(AlignLeft, AlignTop, RectF(0, 0, 32, 32), color4(1, 1, 1, 1), IconFile, 0, TextureDir);
		Text = this->AddText(AlignLeft, AlignTop, szText, color4(1, 1, 1, 1), 24, GetTypeFont(Font_Title));
		Text->Offset.x = 32;
		Text->InvalidText = true;
		...
		:DexianShape(dlg)
	{
		...
		nLineHeight = 24;
		Callback = 0;
		Title = this->AddText(AlignLeftRight, AlignTop, "", color4(1, 1, 1, 1), 24, GetTypeFont(Font_Title));
		Title->TextPtr->SetFontSize(24);
		Title->TextPtr->Redraw();
		Title->SetLayer(1);
		Selected = this->AddRect(AlignLeftRight, AlignTop, RectF(0, 0, 0, nLineHeight), color4(0.01f, 0.8f, 0.5f, 0.8));
		Selected->SetLayer(-1);
		Selected->Display = false;
		HotRect = this->AddRect(AlignLeftRight, AlignTop, RectF(0, 0, 0, nLineHeight), color4(0.01f, 0.2f, 0.5f, 1));
		HotRect->SetLayer(-2);
		HotRect->Display = false;
		Back = this->AddRect(AlignLeftRight, AlignTopBottom, RectF(0, 0, 0, 0), color4(0.01f, 0.01f, 0.01f, 0.5));
		Back->SetLayer(-3);
	}
	...
	VOID				DexianShape::SetWindowRect(const RectF& rc)
	{
		DexianControl::SetWindowRect(rc);
		for (int i = 0; i < Draws.size(); i++) {
			Draws[i]->Invalidate();
		}
	}
	...
	{
		DexianListLine* L = new DexianListLine(this);
		L->SetupLine(Text, IconFile, TextureDir);
		int Top = Lines.size() * nLineHeight + nTitleHeight;
		L->SetClientRect(RectF(0, Top, 0, Top + nTitleHeight));
		L->Context = Context;
		Childs += L;
		Lines += L;
	...
	{
		int index = (pos.y - WindowRect.top - nTitleHeight) / nLineHeight;
		if (!IsPtIn(pos))
			return -1;
		return index;
	}
	char 		DexianListBox::OnMouseMessage(unsigned int uMsg, PixelF pos, unsigned int touchIndex)
	{
		...
		case InputEventID_MouseMove:
		{
			if (bMoveDown == (touchIndex + 1))
			{
				...
				if (hotIndex < 0 || hotIndex >= Lines.size())
					...
					HotRect->Offset.y = nTitleHeight + nLineHeight*hotIndex + 3;
					HotRect->Invalidate();
				}
			}
			else {
		...
		case InputEventID_ButtonUp:
		{
			...
			if (bMoveDown)
			{
				...
				{
					Selected->Display = true;
					Selected->Offset.y = nTitleHeight + nLineHeight*hotIndex + 3;
	...
	void					DexianStatic::OnRender(float fElapsedTime)
	{
		RenderElement(&Back);
	}
	...
	void		DexianButton::OnRender(float fElapsedTime)
	{
		color4 c = Back.Color;
		if (bMoveDown)
			Back.Color *= 0.5f;
	...
	void					DexianGrids::OnRender(float fElapsedTime)
	{
		...
		for (int nX = 0; nX < numX; nX++) {
			...
			if (gIndex != 0)
				g_manager.DrawLines2D(xys, 2, LineColor, false);
		}
		...
		for (int nX = 0; nX < numX; nX++) {
			...
			int gIndex = (nX - xGroup + (OffsetX / nAlign)) % GroupScale;
			if (gIndex == 0)
				g_manager.DrawLines2D(xys, 2, GroupColor, false); //g_manager.DrawRect2D(groupRect, GroupColor, true);
		}
		for (int nY = 0; nY < numY; nY++) {
			xys[0].Set(OffsetX, yBegin + nY*nAlign + OffsetY);
			xys[1].Set(OffsetX + Width, yBegin + nY*nAlign + OffsetY);
	...
	void	DexianManager::CreateTooltip()
	{
		if (m_toolTip)
			delete m_toolTip;
		m_toolTip = new DexianShape(0);
		m_toolTip->SetClientRect(RectF(0, 0, 128, 24));
		m_toolTip->AddRect(AlignLeft, AlignTop, RectF(0, 0, 128, 24), color4(0, 0, 0, 0.1f));
		m_toolTipText = m_toolTip->AddText(AlignLeft, AlignTop, "TEST ToolTip", color4(1, 1, 0.1, 1.0f), 24);
	}
	...
	Font*							DexianManager::GetFont(PtrID& id)
	{
	...
	DexianFont*						DexianManager::NewFont(const char* szFontName, int height, int weight, char italic, char underline)
	{
		CPUTime(UIManager);
		if (!szFontName)
			szFontName = "Arial";
	...
	DexianTexture*					DexianManager::NewTexture(int TextureIndex, int filter, BaseDirectory* dir)
	{
		CPUTime(UIManager);
	...
	DexianControl*					DexianManager::NewControl(DexianControlData::ControlType Type)
	{
		DexianControl* cc = CreateControl(Type, 0);
		if (cc)
	...
	void							DexianManager::Render(float fElapsedTime)
	{
		for (int i = (int)m_controlMap.size() - 1; i >= 0; i--) {
			...
			if(m_controlMap[i]->Display)
				m_controlMap[i]->Render(fElapsedTime);
		}
	}
