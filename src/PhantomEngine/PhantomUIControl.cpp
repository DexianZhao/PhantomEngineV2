...
{CPUTime(UIControl);
	...
	if(version<0x000000A3){
		...
		if(tiplength > 0)
		{
			TypeArray<short_t> szToolTip;
			szToolTip.resize(tiplength + 1);
...
void		RenderImageText(const Rect& windowRect, const char* numberBegin, const char* szText, int textLength, int textureID, int align,
	const color4& color, const float2& scale, const Pixel& rotcenter, TypeArray<int>& indexs, int imageSize, int imageOffset, const Pixel* offsetXY, BOOL bIsEnabled, int cullmode)
{
	TypeArray<TextureRect>* rects = GetDlgManager()->GetTextureRectPtr(textureID);
	if(!rects)return;
  ...
  //          Global_playsnd(m_soundFile, 1, 0, 1);
	...
	{CPUTime(UIDrawMgr);
		...
		if(tr)
		{
			...
			SetRect(tr->off.x, tr->off.y, tr->off.x+tr->rc.Width(), tr->off.y+tr->rc.Height());
		}
	}
