...
namespace Phantom{
	...
	PHANTOM_API BOOL	LoadJpeg(ImageBuffer& ret, const void* dataPtr, int dataLength)
	{
...
//						assert(index <= (ret.width * ret.height * 4));
	...
	PHANTOM_API VOID	LoadPng(const char* s)
	{
...
//		if(m_pos >= 255 || index >= m_nNumVertices)
//			return;
//		if(m_posV.size() == 0)m_posV.resize(m_nNumVertices);
//		m_posV[index]	=	pos;
//		if(this->m_bIs2DMode)
...
//		if(m_normal >= 255 || index >= m_nNumVertices)
//			return;
//		if(m_normalV.size() == 0)m_normalV.resize(m_nNumVertices);
...
//		if(m_diffuse >= 255 || index >= m_nNumVertices)
//			return;
//		if(m_diffuseV.size() == 0)m_diffuseV.resize(m_nNumVertices);
...
//		if(m_specular >= 255 || index >= m_nNumVertices)
//			return;
//		if(m_specularV.size() == 0)m_specularV.resize(m_nNumVertices);
...
//		if(m_uv >= 255 || index >= m_nNumVertices)
//			return;
//		if(m_uvV.size() == 0)m_uvV.resize(m_nNumVertices);
//		m_uvV[index]	=	pos;
//	}
//	VOID		DynamicMesh::SetUV2(unsigned int index, const float2& pos)
//	{
//		if(m_uv2 >= 255 || index >= m_nNumVertices)
//			return;
//		if(m_uv2V.size() == 0)m_uv2V.resize(m_nNumVertices);
//		m_uv2V[index]	=	pos;
//	}
//	VOID		DynamicMesh::SetUV3(unsigned int index, const float2& pos)
//	{
//		if(m_uv3 >= 255 || index >= m_nNumVertices)
//			return;
//		if(m_uv3V.size() == 0)m_uv3V.resize(m_nNumVertices);
//		m_uv3V[index]	=	pos;
//	}
//	VOID		DynamicMesh::SetUV4(unsigned int index, const float2& pos)
//	{
//		if(m_uv4 >= 255 || index >= m_nNumVertices)
//			return;
//		if(m_uv4V.size() == 0)m_uv4V.resize(m_nNumVertices);
//		m_uv4V[index]	=	pos;
//	}
//	VOID		DynamicMesh::SetUV5(unsigned int index, const float2& pos)
//	{
//		if(m_uv5 >= 255 || index >= m_nNumVertices)
//			return;
//		if(m_uv5V.size() == 0)m_uv5V.resize(m_nNumVertices);
//		m_uv5V[index]	=	pos;
//	}
//	BOOL					DynamicMesh::GetPos(unsigned int index, float3& ret)
//	{
...
//		{
//			m_triangleAttrs.resize(m_triangles.size());
//		}
...
//		TypeArray<Triangle>*		temps = new TypeArray<Triangle>[m_attributes.size()];
//		m_trianglesRenders.resize(m_triangles.size());
//		for(int i=0;i<m_triangleAttrs.size();i++)
//		{
//			TypeArray<Triangle>& t = temps[m_triangleAttrs[i]];
...
//	{
//		m_triangles.resize(indiceCount / 3);
//		if(indiceCount > 0)
...
//#ifdef DIRECT_X
//		m_vertices.resize(m_nStrideSize * m_nNumVertices);
//#else
//		//m_posV.resize(m_nNumVertices);
//		//m_normalV.resize(m_nNumVertices);
//		//m_diffuseV.resize(m_nNumVertices);
//		//m_specularV.resize(m_nNumVertices);
//		//m_uvV.resize(m_nNumVertices);
//		//m_uv2V.resize(m_nNumVertices);
//		//m_uv3V.resize(m_nNumVertices);
//		//m_uv4V.resize(m_nNumVertices);
//		//m_uv5V.resize(m_nNumVertices);
//
//#endif
//	}
...
//		if(m_triangleAttrs.size() > 0)
//			m_triangleAttrs.resize(numTriangles);
//	}
...
//
	...
	VOID			ImageBuffer::AutoResize(const char* szFile)//自动重新计算贴图大小,这会消耗一定的时间,并给出提示
	{
		...
		for (int y = 0; y < oriheight; y++) {
			unsigned char*	src = pixels + (y*oriwidth)*bppWidth;
			unsigned char*	dest = newpixels + (y*width)*bppWidth;
			memcpy(dest, src, oriwidth*bppWidth);
		}
...
//#endif
	...
	PHANTOM_API int	GetNearestSize(int n){
		if(n<=8)return 8;
		if(n<=16)return 16;
		if(n<=32)return 32;
		if(n<=64)return 64;
...
/*
	...
	void				RenderText::Redraw()
	{
		if (g_manager.IsDeviceSetup())
			OnResetDevice();
	}

	RenderEditText::RenderEditText() {
		Align = 0;
		Select = Cursor = 0;
	...
	VOID				RenderEditText::SetFontSize(int nSize)
	{
		CPUTime(RenderEditText);
		m_nFontSize = nSize;
	}
	VOID				RenderEditText::SetFontStyle(int nStyle)
	{
		CPUTime(RenderEditText);
		m_nFontStyle = nStyle;
	}
	VOID				RenderEditText::SetFontName(const char* name)
	{
		if (name)
			m_fontName = name;
		else
	...
	{CPUTime(DeviceMesh);
		...
		if(m_nNumVertices > 0)
			m_vertices.resize(m_nNumVertices * m_nStrideSize);
		if(m_nNumIndices > 0)
		{
			m_indices.resize(m_nNumIndices * sizeof(WORD));
			m_attributes.resize(nNumFaces);
		}
	}
	...
	{CPUTime(DeviceMesh);
		...
		if(attrib)
		{
			m_attributes.resize(indexCount / 3);
	...
	VOID				InsertLine(TypeArray<char>& v, int & index, const char* szText)
	{
	...
	{CPUTime(Program);
		...
		//
		m_vsps_directx9.resize(len + 5000);
		m_vsps_opengles_v.resize(len + 5000);
		m_vsps_opengles_p.resize(len + 5000);
		int vsps_directx9	=	0;
		...
		m_vsps_directx9.resize(vsps_directx9);
		m_vsps_opengles_v.resize(vsps_opengles_v);
		m_vsps_opengles_p.resize(vsps_opengles_p);
		return true;
	}
