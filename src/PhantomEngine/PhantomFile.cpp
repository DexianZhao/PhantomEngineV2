...
extern "C"
{
	...
	{
		Phantom::TypeArray<Phantom::FileName>* rets = (Phantom::TypeArray<Phantom::FileName>*)ptr;
		Phantom::FileName f;
		f = szFile;
		//f.setIsFile();
		rets->push_back(f);
...
namespace Phantom{
	...
	void								searchFiles(TypeArray<FileName>& rets, const char* documents, const char* szSubFolders, int subfolder)
	{
	...
	BOOL			open_s_full(const char* szFile, TypeArray<char>& rets)	//load file from szFile, return array
	{
	...
	PHANTOM_API BOOL				LoadUtf8(const char* szFile, TypeArray<char>& ret)
	{
	...
	PHANTOM_API VOID				SaveUtf8(const char* szFile, const char* text)
	{
	...
	PakFileData*	LoadFileData(const char* szFile, BOOL bCompress){
		...
		if(bCompress&&data->buffers.size()>1024){
			TypeArray<char> temp = data->buffers;
	...
	BOOL	IsExistFile(TypeArray<PakFileData*>& files, const char* szFile)
...
#define	PAK_FILE_HEADER		_MakeFileHead('p','a','k','1')
	...
	char				PakDirectory::ExportPak(const char* szFile, TypeArray<PakFileData*>& files)//, const unsigned char* pass, int passlen)
	{
		TypeArray<PakFileInfo> infos;
	...
	BOOL				PakDirectory::ResetPassword(TypeArray<char>& dest, TypeArray<char>& src, const unsigned char* oldpass, int oldpasslen)
	{
		GenuineGUID temp;
