...
//#endif
	...
	{CPUTime(UVAnimation);
		...
		m_nFrameCount	=	nEnd;
		TypeArray<TextureRectUV> rcTemp	=	m_rects;
		if(m_nFrameCount > 0)
		{
			m_rects.resize(m_nFrameCount);
			memset(m_rects.GetPtr(), 0, sizeof(TextureRectUV) * m_rects.size());
...
//		m_name			=	"n/a";
	...
	{CPUTime(UIElement);
		TypeArray<TextureRect>*	list	=	GetDlgManager()->GetTextureRectPtr(textureIndex);
		if(!list || !list->size())
			return;
	...
	{CPUTime(UIElement);
		...
		if(old > 0)
			GetDlgManager()->DelTexture(old);
		this->m_controlPtr->m_bChanged	=	true;
		this->m_controlPtr->m_textIndexs.resize(0);
	}
