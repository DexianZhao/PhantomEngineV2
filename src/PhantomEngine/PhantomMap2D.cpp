...
namespace Phantom{
	...
	//	TypeArray<Pixel>* pixels = new TypeArray<Pixel>[maxSize];
	//	for(int x=0;x<m_tileCount.cx;x++){
	//		for(int y=0;y<m_tileCount.cy;y++)
	//		{
	//			int xy = x + y;
	//			TypeArray<Pixel>& pd = pixels[xy];
	//			pd.push_back(Pixel(x,y));
	//		}
	//	}
	//	m_tilesIds.resize(m_tileCount.cx * m_tileCount.cy);
	//	int index = 0;
	//	for(int i=0;i<maxSize;i++)
	//	{
	//		TypeArray<Pixel>& pd = pixels[i];
	...
	//TypeArray<Map2D_Event*>		m_events;
	...
	{CPUTime(Map2D_2x1);
		...
		if(this->m_pathSize.cx*this->m_pathSize.cy>0)
			m_pathPoints.resize(this->m_pathSize.cx*this->m_pathSize.cy);
	...
	BOOL					Map2D_2x1::FindPath(TypeArray<WayPoint2D>& ways, const Pixel& mapFrom, const Pixel& mapTo)
	{CPUTime(Map2D_2x1);
	...
	{CPUTime(Map2D_2x1);
		TypeArray<MapItem_Unit*> deletes;
		for(int i=0;i<m_units.size();i++)
		{
	...
	{CPUTime(Map2D_2x1);
		TypeArray<PathPoint>			old;
		old = m_pathPoints;
		Size oldsize = m_pathSize;
		m_pathSize.Set(w,h);
		if(w*h>0)
		{
			m_pathPoints.resize(w*h);
