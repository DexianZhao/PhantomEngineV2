...
#define	LEADER_ROLE_ID	1//主角roleID
	...
	{CPUTimeT(GameMgr);
		m_inobjects.resize(m_objects.size());
		int count = 0;
	...
	VOID			ReplaceFunction(TypeArray<char>& ret, const char* szText, const char* szFinds, const char* szReplace, TypeArray<Name>& funcs){
	...
	bool	GameMgr::AddScripts(ScriptBase::eScript type, TypeArray<ScriptBase*>& sc, const char* config, const char* func, int loadID)
	{CPUTimeT(GameMgr);
