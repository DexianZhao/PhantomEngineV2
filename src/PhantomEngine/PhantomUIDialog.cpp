...
//				loadRect.bottom = loadRect.top + loadH;
	...
	struct	LineObjects
	{
		TypeArray<LineItem>	objs;//1000*256*6*4
	};
...
//		pStatic->SetText( strText );
	...
	struct	ControlVar{
		...
		int		id;
		short	isary;//-1
		TypeArray<ControlVarAry> arys;
	};
	struct	GroupVar{
		TypeArray<ControlVar*>	vars;
		Name	name;
		int		id;
		~GroupVar(){
...
#define CURRENT_UI_VERSION	0x0000088
	...
	extern	BOOL	IsExistFile(TypeArray<PakFileData*>& files, const char* szFile);
	...
	{CPUTime(UIDialog);
		...
		{
			TypeArray<Map2DGridInfo> olds	=	m_mapGrids;
			int cx = m_mapGridCount.cx;
			int cy = m_mapGridCount.cy;
	...
	PHANTOM_API VOID			ResetCenter(UIDialog* dlg){
		if(!dlg->m_bMapEnable)
			return;
	...
	Map2DGridInfo*	GetGridInfo(TypeArray<Map2DGridInfo>& olds, Size& oldGridSize, Size& oldSize, Size& newGridSize, int x, int y)
	{
