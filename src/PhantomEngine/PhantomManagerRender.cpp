...
namespace Phantom{
	...
	{CPUTimeT(Manager);
		...
		if(!SetRenderMode(RenderMode_Text))
			return false;
		...
		rect_f screen((float)drr.left / (float)this->m_bufferWidth, (float)drr.top / (float)this->m_bufferHeight, (float)drr.right 
			/ (float)this->m_bufferWidth, (float)drr.bottom / (float)this->m_bufferHeight);
...
VOID		Manager::DrawCircle( const PixelF& center, float fRadius, const color4& c, int smooth, BOOL bFill)//渲染一个圆
{
	CPUTimeT(Manager);
	float left = (float)((float)(-m_uiOffset.x * (float)GetBufferWidth()) / m_uiScale.x);
	float right = (float)((float)(GetBufferWidth() - m_uiOffset.x * (float)GetBufferWidth()) / m_uiScale.x);
	float top = (float)((float)(-m_uiOffset.y * (float)GetBufferHeight()) / m_uiScale.y);
	float bottom = (float)((float)(GetBufferHeight() - m_uiOffset.y * (float)GetBufferHeight()) / m_uiScale.y);
	if ((center.x + fRadius) < left || (center.x - fRadius) > right || (center.y + fRadius) < top || (center.y - fRadius) > bottom)
		return;
	float s = m_uiScale.x;
	if (s < 0.5f)s = 0.5f;
	smooth = (int)((float)smooth*s);
	INT nNumPoints	=	smooth + 1;
	if(nNumPoints <= 1)return;
	if(!SetRenderMode(RenderMode_Color))
		return;
	m_device->SetCullVisible(CullMode_CCW);
	//
	m_tempPosList.resize(nNumPoints + 5);
	m_tempColorList.resize(nNumPoints + 5);
	double fZ	=	0;
	double fY	=	m_fTan2DFov;
	double fX	=	fY * (double)this->m_bufferWidth / (double)this->m_bufferHeight;
	unsigned int temp	=	c;
	for( unsigned int i=0; i <= smooth+1; i++ )
	{
	...
	if (bFill)
	{
		//m_tempPosList[smooth+1] = m_tempPosList[0];
		//m_tempColorList[smooth+1] = m_tempColorList[0];
		float x = (float)center.x / (float)this->m_bufferWidth;
		float y = (float)center.y / (float)this->m_bufferHeight;
		x *= m_uiScale.x;
		y *= m_uiScale.y;
		x += m_uiOffset.x;
		y += m_uiOffset.y;
		m_tempPosList[0].x = (x - 0.5f) * fX;
		m_tempPosList[0].y = (0.5f - y) * fY;
		nNumPoints++;
...
VOID			Manager::DrawLines2D(const PixelF* xys, int count, const color4& c, BOOL bFill/*是否填充*/, float FillWidth)	//渲染2D线条
{
	CPUTimeT(Manager);
	if (!SetRenderMode(RenderMode_Color))
		return;
	if (bFill) {
		m_device->SetCullVisible(CullMode_CCW);
		m_tempPosList.resize(count*3);
		m_tempColorList.resize(count*3);
		m_tempFaceList.resize((count-1) * 2 * 2);
		double fZ = 0;
		double fY = m_fTan2DFov;
		double fX = fY * (double)this->m_bufferWidth / (double)this->m_bufferHeight;
		unsigned int temp = c;
		unsigned int alpha = temp & 0x00ffffff;
		for (int i = 0; i < count; i++)
		{
			float x = (float)xys[i].x;
			float y = (float)xys[i].y;
			float dx, dy;
			if (i < (count - 1))
			{
				dx = xys[i + 1].x - x;
				...
				dx = xys[i].x - xys[i - 1].x;
				dy = xys[i].y - xys[i - 1].y;
			}
			m_tempPosList[i].setxyz(x, y, 0);
			PixelF cross(dy, -dx);
			cross.normalize();
			cross *= FillWidth;
			m_tempPosList[i].setxyz(x + cross.x, y + cross.y, 0);
			m_tempPosList[i + count].setxyz(x, y, 0);
			m_tempPosList[i + count * 2].setxyz(x - cross.x, y - cross.y, 0);
			m_tempColorList[i] = alpha;
			m_tempColorList[i + count] = temp;
			m_tempColorList[i + count * 2] = alpha;
			if (i < (count - 1)) {
				m_tempFaceList[i * 4 + 0].set(i + count, i, i + 1);
				m_tempFaceList[i * 4 + 1].set(i + 1, i + 1 + count, i + count);
				m_tempFaceList[i * 4 + 2].set(i + count * 2, i + count, i + 1 + count);
				m_tempFaceList[i * 4 + 3].set(i + 1 + count, i + 1 + count * 2, i + count * 2);
			}
		}
		for(int i=0;i<m_tempPosList.size();i++){
			float3& p = m_tempPosList[i];
			p.x = p.x / (float)this->m_bufferWidth * m_uiScale.x + m_uiOffset.x;
			p.y = p.y / (float)this->m_bufferHeight * m_uiScale.y + m_uiOffset.y;
			p.x = (p.x - 0.5f) * fX;
			p.y = (0.5f - p.y) * fY;
			p.z = fZ;
		}
		SetWorldMatrix2D(0);
		m_device->DrawIndexed(m_currProg, DrawVInfos(m_tempPosList.size(), m_tempPosList.GetPtr(), m_tempColorList.GetPtr()), (unsigned short*)m_tempFaceList.GetPtr(),
			m_tempFaceList.size()*3, DrawMode_Triangles);
	}
	else {
		m_device->SetCullVisible(CullMode_CW);
		m_tempPosList.resize(count);
		m_tempColorList.resize(count);
		double fZ = 0;
		double fY = m_fTan2DFov;
		double fX = fY * (double)this->m_bufferWidth / (double)this->m_bufferHeight;
		unsigned int temp = c;
...
VOID			Manager::DrawLines2D(const Pixel* xys, int count, const color4& c, BOOL bFill/*是否填充*/)	//渲染2D线条
	...
	VOID			Manager::DrawRoundRect2D(const RectF& rc, float fRadius, int nMode, const color4* c4, BOOL bFill/*是否填充*/, BOOL bLingXing)
	{
		CPUTimeT(Manager);
		if (!SetRenderMode(RenderMode_Color))
			return;
		...
		if (rc.right<left || rc.left>right || rc.bottom<top||rc.top>bottom)
			return;
			//(float)((float)(m_cursorPos[index].y - m_uiOffset.y*(float)GetBufferHeight()) / m_uiScale.y));
		...
		//screen.left = screen.left*m_uiScale.x + m_uiOffset.x;
		//screen.top = screen.top*m_uiScale.y + m_uiOffset.y;
		//screen.right = screen.right*m_uiScale.x + m_uiOffset.x;
		//screen.bottom = screen.bottom*m_uiScale.y + m_uiOffset.y;
		//if (screen.bottom > 1.0f)
		//	return;
		//if (screen.top < 0.0f)
		//	return;
		//if (screen.right > 1.0f)
		//	return;
		//if (screen.left < 0.0f)
		//	return;
		m_device->SetCullVisible(CullMode_CCW);
		...
		//double b = (0.5f - screen.bottom) * fY;
		float2 Points[4];
		float Rotation[4] = { PI, PI / 2.0f*3.0f, 0, PI / 2.0f };// PI / 2.0f, PI, PI / 2.0f*3.0f};
		Points[0] = float2(rc.left + fRadius, rc.top + fRadius);
		Points[1] = float2(rc.right - fRadius, rc.top + fRadius);
		Points[2] = float2(rc.right - fRadius, rc.bottom - fRadius);
		Points[3] = float2(rc.left + fRadius, rc.bottom - fRadius);
		float centerX = (rc.left + rc.right) / 2.0f;
		float centerY = (rc.top + rc.bottom) / 2.0f;
		int sec = smooth + 2;
		m_tempPosList[0].setxyz(centerX, centerY, 0);
		m_tempPosList[sec * 0 + 1].setxyz(rc.left, centerY, 0);
		m_tempPosList[sec * 1 + 1].setxyz(centerX, rc.top, 0);
		m_tempPosList[sec * 2 + 1].setxyz(rc.right, centerY, 0);
		m_tempPosList[sec * 3 + 1].setxyz(centerX, rc.bottom, 0);
		m_tempPosList[sec * 4 + 1].setxyz(rc.left, centerY, 0);
		unsigned int temp = c4[0];
		for (unsigned int i = 0; i <= smooth; i++)
		{
			for (int j = 0; j < 4; j++) {
				float f = (float)i / (float)smooth * (PI / 2.0f) + Rotation[j];
				float x = cosf(f)*fRadius + Points[j].x;
				float y = sinf(f)*fRadius + Points[j].y;
				m_tempPosList[sec*j + 2 + i].setxyz(x, y, 0);
			}
		}
	...
	VOID			Manager::DrawRect2D(const RectF& rc, const color4& c, BOOL bFill/*是否填充*/, BOOL bLingXing)	//渲染2D线条
	{
		CPUTimeT(Manager);
		if(!SetRenderMode(RenderMode_Color))
			return;
	...
	VOID		Manager::RenderRect2D(DrawBuffer* buffer, const RectF& rc, const color4& c, bool bFill)
	{
		CPUTimeT(Manager);
		if (rc.right<m_uiClipWindow.left || rc.left>m_uiClipWindow.right || rc.bottom<m_uiClipWindow.top || rc.top>m_uiClipWindow.bottom)
			return;
		m_device->SetAlphaMode(Phantom::AlphaMode_Blend);
		m_device->SetBlendMode(Phantom::BlendMode_SRCALPHA, Phantom::BlendMode_INVSRCALPHA);
		if (!SetRenderMode(RenderMode_ColorBuffer))
			return;
		m_device->SetCullVisible(CullMode_CW);
		if (!buffer->Valid || m_bSizeChanged) {
			rect_f screen((float)rc.left / (float)this->m_bufferWidth, (float)rc.top / (float)this->m_bufferHeight,
				(float)rc.right / (float)this->m_bufferWidth, (float)rc.bottom / (float)this->m_bufferHeight);
			//screen.left = screen.left*m_uiScale.x + m_uiOffset.x;
			//screen.top = screen.top*m_uiScale.y + m_uiOffset.y;
			...
			//double b = (0.5f - screen.bottom) * fY;
			//
			pos[0].setxyz(screen.left, screen.top, fZ);
			pos[1].setxyz(screen.right, screen.top, fZ);
			pos[2].setxyz(screen.right, screen.bottom, fZ);
			pos[3].setxyz(screen.left, screen.bottom, fZ);
			unsigned int temp = c;
			for (int i = 0; i < 4; i++)
				color[i] = temp;
			static unsigned short indexBuffer[] = { 3,1,0,3,2,1 };
			static unsigned short indexBufferLine[] = { 0,1,1,2,2,3,3,0 };
			if(bFill)
				m_device->MakeDrawBuffer(buffer, DrawVInfos(4, pos, color), indexBuffer, 6, DrawMode_Triangles);
			else
				m_device->MakeDrawBuffer(buffer, DrawVInfos(4, pos, color), indexBufferLine, 8, DrawMode_Lines);
		}
		SetWorldMatrix2D(0);
		m_device->Draw(m_currProg, buffer);
	}
	BOOL		Manager::IsRenderVisible(const RectF& rc)
	{
		if (rc.right<m_uiClipWindow.left || rc.left>m_uiClipWindow.right || rc.bottom<m_uiClipWindow.top || rc.top>m_uiClipWindow.bottom)
			return false;
		return true;
	}
	void		Manager::RenderTexture(DrawBuffer* buffer, Texture* tex, const RectF& rc, const RectF& texRc, const color4& c)
	{
		if (rc.right<m_uiClipWindow.left || rc.left>m_uiClipWindow.right || rc.bottom<m_uiClipWindow.top || rc.top>m_uiClipWindow.bottom)
			return;
		m_device->SetAlphaMode(Phantom::AlphaMode_Blend);
		m_device->SetBlendMode(Phantom::BlendMode_SRCALPHA, Phantom::BlendMode_INVSRCALPHA);
		if (!SetRenderMode(RenderMode_ImageBuffer))
			return;
		m_device->SetCullVisible(CullMode_CW);
		if (!buffer->Valid || m_bSizeChanged) {
			rect_f screen((float)rc.left, (float)rc.top, (float)rc.right, (float)rc.bottom);
			float3 pos[4];
			float2 uv[4];
			unsigned int color[4];
			float fZ = 0;// buffer->Depth;
			//double fY = m_fTan2DFov;
			//double fX = fY * (double)this->m_bufferWidth / (double)this->m_bufferHeight;
			//double l = (screen.left - 0.5f) * fX;
			//double t = (0.5f - screen.top) * fY;
			...
			//double b = (0.5f - screen.bottom) * fY;
			//
			float centerX = (float)(rc.left + buffer->Center.x / texRc.Width() * rc.Width());
			float centerY = (float)(rc.top + buffer->Center.y / texRc.Height() * rc.Height());
			pos[0].setxyz(screen.left, screen.top, fZ);
			pos[1].setxyz(screen.right, screen.top, fZ);
			pos[2].setxyz(screen.right, screen.bottom, fZ);
			pos[3].setxyz(screen.left, screen.bottom, fZ);
			for (int i = 0; i < 4; i++) {
				float3 t = pos[i] - float3(centerX, centerY, fZ);// .RotateZ()
				t.x *= buffer->Scale;
				t.y *= buffer->Scale;
				pos[i].x = t.x + centerX;
				pos[i].y = t.y + centerY;
			...
			for (int i = 0; i < 4; i++) {
				float3 t = pos[i] - float3(centerX, centerY, fZ);// .RotateZ()
				t.RotateZ(t, buffer->Rotation);
				pos[i].x = t.x + centerX;
				pos[i].y = t.y + centerY;
			}
			for (int i = 0; i < 4; i++) {
				pos[i].x /= (float)m_bufferWidth;
				pos[i].y /= (float)m_bufferHeight;
			}
			float TextureWidth = tex->GetWidth();
			float TextureHeight = tex->GetHeight();
			//
			uv[0] = float2(texRc.left / TextureWidth, texRc.top / TextureHeight);
			uv[1] = float2(texRc.right / TextureWidth, texRc.top / TextureHeight);
			uv[2] = float2(texRc.right / TextureWidth, texRc.bottom / TextureHeight);
			uv[3] = float2(texRc.left / TextureWidth, texRc.bottom / TextureHeight);
			unsigned int temp = c;
			for (int i = 0; i < 4; i++)
				color[i] = temp;
			static unsigned short indexBuffer[] = { 3,1,0,3,2,1, 0,1,3,1,2,3 };
			int numIndices = 6;
			m_device->MakeDrawBuffer(buffer, DrawVInfos(4, pos, color, uv), indexBuffer, numIndices, DrawMode_Triangles);
		}
		SetWorldMatrix2D(0);
		if (tex)
			this->SetTexture(tex);
	...
	VOID		Manager::RenderText2D(DrawBuffer* buffer, RenderText* textPtr, const RectF& rc, const color4& c, const RectF* TextRect)
	{
		if (rc.right<m_uiClipWindow.left || rc.left>m_uiClipWindow.right || rc.bottom<m_uiClipWindow.top || rc.top>m_uiClipWindow.bottom)
			return;
		if (!textPtr || textPtr->IsEmpty() || !textPtr->IsInit())return;
		m_device->SetCullVisible(Phantom::CullMode_CW);
		m_device->SetAlphaMode(Phantom::AlphaMode_Blend);
		m_device->SetBlendMode(Phantom::BlendMode_SRCALPHA, Phantom::BlendMode_INVSRCALPHA);// ONE);
		if (!SetRenderMode(RenderMode_TextBuffer))
			return;
		if (!buffer->Valid || m_bSizeChanged) {
			Rect textRect = textPtr->GetTextureRect();
			if (TextRect)
				textRect = *TextRect;
			rect_f screen((float)rc.left / (float)m_bufferWidth, (float)rc.top / (float)m_bufferHeight,
				(float)rc.right / (float)m_bufferWidth, (float)rc.bottom / (float)m_bufferHeight);
			int tWidth = textPtr->GetRealWidth();
			int tHeight = textPtr->GetRealHeight();
#ifdef DIRECT_X_9
			float offT = 0.5f;
			...
			float offT = 0;
#endif
			//float fS = (float)textRect.right / tHeight;
			//float fRC = rc.GetWidth() / rc.GetHeight();
			//if (fS > fRC)
			//	textRect.right = fRC * tHeight;
			rect_f texRc((float)((float)textRect.left + offT) / tWidth, (float)((float)textRect.top + offT) / tHeight, (float)((float)textRect.right + offT) / tWidth, (float)((float)textRect.bottom + offT) / tHeight);
			...
			float fZ = 0;
			//double fY = m_fTan2DFov;
			//double fX = fY * (double)m_bufferWidth / (double)m_bufferHeight;
			//double l = (screen.left - 0.5f) * fX;
			//double t = (0.5f - screen.top) * fY;
			//double r = (screen.right - 0.5f) * fX;
			//double b = (0.5f - screen.bottom) * fY;
	...
	VOID		Manager::RenderText2D(DrawBuffer* buffer, RenderEditText* textPtr, const RectF& rc, const color4& c, const RectF* TextRect)
	{
		if (rc.right<m_uiClipWindow.left || rc.left>m_uiClipWindow.right || rc.bottom<m_uiClipWindow.top || rc.top>m_uiClipWindow.bottom)
			return;
		if (!textPtr || (textPtr->IsEmpty() && !textPtr->bShowCaret) || !textPtr->IsInit())return;
		m_device->SetCullVisible(Phantom::CullMode_CW);
		m_device->SetAlphaMode(Phantom::AlphaMode_Blend);
		m_device->SetBlendMode(Phantom::BlendMode_SRCALPHA, Phantom::BlendMode_INVSRCALPHA);// ONE);
		if (!SetRenderMode(RenderMode_TextBuffer))
	...
	VOID		Manager::RenderRoundRect2D(DrawBuffer* buffer, const RectF& rc, float fRadius, int nMode/*1=lefttop,2=righttop,4=leftbottom,8=rightbottom*/, const color4* c4)
	{
		CPUTimeT(Manager);
		if (rc.right<m_uiClipWindow.left || rc.left>m_uiClipWindow.right || rc.bottom<m_uiClipWindow.top || rc.top>m_uiClipWindow.bottom)
			return;
		...
		if (!buffer->Valid || m_bSizeChanged) {
			...
			Points[3] = float2(rc.left + fRadius, rc.bottom - fRadius);
			float2 Ext[4];
			Ext[0] = float2(rc.left, rc.top);
			Ext[1] = float2(rc.right, rc.top);
			Ext[2] = float2(rc.right, rc.bottom);
			Ext[3] = float2(rc.left, rc.bottom);
			float ExtR = fRadius / 20.0f;
			float2 ExtFrom[4];
			ExtFrom[0] = float2(0, ExtR);
			ExtFrom[1] = float2(-ExtR, 0);
			ExtFrom[2] = float2(0, -ExtR);
			ExtFrom[3] = float2(ExtR, 0);
			float2 ExtTo[4];
			ExtTo[0] = float2(ExtR, 0);
			ExtTo[1] = float2(0, ExtR);
			ExtTo[2] = float2(-ExtR, 0);
			ExtTo[3] = float2(0, -ExtR);
			float centerX = (rc.left + rc.right) / 2.0f;
			float centerY = (rc.top + rc.bottom) / 2.0f;
			int sec = smooth + 2;
			m_tempPosList[0].setxyz(centerX, centerY, 0);
			...
			for (int j = 0; j < 4; j++) {
				...
				if ((nMode&n) == n) {
					...
					unsigned int half = smooth / 2;
					for (unsigned int i = 0; i < half; i++)
					{
						float f = (float)i / (float)half;
						float2 dst = ExtFrom[j] * (1.0f-f) + Ext[j];
						m_tempPosList[sec*j + 2 + i].setxyz(dst.x, dst.y, 0);
					}
					for (unsigned int i = 0; i <= half; i++)
					{
						float f = (float)i / (float)half;
						float2 dst = ExtTo[j] * f + Ext[j];
						m_tempPosList[sec*j + 2 + i + half].setxyz(dst.x, dst.y, 0);
					}
				}
			}
			for (int i = 0; i < nNumPoints; i++) {
			...
			m_device->MakeDrawBuffer(buffer, DrawVInfos(nNumPoints, m_tempPosList.GetPtr(), m_tempColorList.GetPtr()), DrawMode_TriangleFan);
		}
		SetWorldMatrix2D(0);
		m_device->Draw(m_currProg, buffer);
	}
	VOID		Manager::RenderLines2D(DrawBuffer* buffer, const RectF& rc, const PixelF* xys, int count, const color4& c, float FillWidth)
	{
		CPUTimeT(Manager);
		if (rc.right<m_uiClipWindow.left || rc.left>m_uiClipWindow.right || rc.bottom<m_uiClipWindow.top || rc.top>m_uiClipWindow.bottom)
			return;
		...
		if (!buffer->Valid || m_bSizeChanged) {
			...
			m_device->MakeDrawBuffer(buffer, DrawVInfos(m_tempPosList.size(), m_tempPosList.GetPtr(), m_tempColorList.GetPtr()), (unsigned short*)m_tempFaceList.GetPtr(),
				m_tempFaceList.size() * 3, DrawMode_Triangles);
		}
		SetWorldMatrix2D(0);
		m_device->Draw(m_currProg, buffer);
	}
	VOID		Manager::RenderCircle2D(DrawBuffer* buffer, const PixelF& center, float fRadius, const color4& c, int smooth)
	{
		CPUTimeT(Manager);
		if ((center.x + fRadius) < m_uiClipWindow.left || (center.x - fRadius) > m_uiClipWindow.right || (center.y + fRadius) < m_uiClipWindow.top || (center.y - fRadius) > m_uiClipWindow.bottom)
			return;
		m_device->SetAlphaMode(Phantom::AlphaMode_Blend);
		m_device->SetBlendMode(Phantom::BlendMode_SRCALPHA, Phantom::BlendMode_INVSRCALPHA);
		if (!SetRenderMode(RenderMode_ColorBuffer))
		...
		if (!buffer->Valid || m_bSizeChanged) {
			...
			for (unsigned int i = 0; i <= smooth + 1; i++)
			{
				...
				//x += m_uiOffset.x;
				//y += m_uiOffset.y;
				m_tempPosList[i].x = x;// (x - 0.5f) * fX;
				m_tempPosList[i].y = y;// (0.5f - y) * fY;
				m_tempPosList[i].z = fZ;
				m_tempColorList[i] = temp;
			}
			float x = (float)center.x / (float)this->m_bufferWidth;
			...
			//x += m_uiOffset.x;
			//y += m_uiOffset.y;
			m_tempPosList[0].x = x;// (x - 0.5f) * fX;
			m_tempPosList[0].y = y;// (0.5f - y) * fY;
			nNumPoints++;
			m_device->MakeDrawBuffer(buffer, DrawVInfos(nNumPoints, m_tempPosList.GetPtr(), m_tempColorList.GetPtr()), DrawMode_TriangleFan);
		}
		SetWorldMatrix2D(0);
	...
	BOOL			Manager::DrawTexts(const char* szUTF8, Rect& destRect, int nLength, float fontSize, float fItalic, const color4* c, unsigned int dtFormat, const matrix4x4* world)
	{
		CPUTimeNone();
		if (!m_tempText) {
			m_tempText = m_device->CreateRenderText();// Fig();//
		}
        m_tempText->SetFontSize(fontSize);
        m_tempText->SetFontStyle(RenderText::Bold);
