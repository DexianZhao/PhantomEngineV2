...
namespace Phantom{
	...
	{
		m_frees.setGrow(100);
		m_allocs.setGrow(100);
		m_recvs.setGrow(100);
		m_tempRecvs.setGrow(100);
		m_trecvs.setGrow(100);
		m_tempTRecvs.setGrow(100);
		m_bPause			=	false;
		m_fFrameSpeed		=	1;
		...
		m_device				=	0;
		CursorMode = Cursor_Arrow;
#ifdef EXPORT_ENGINE
		m_uiManager				=	new UIManager();
		m_camera				=	new Camera();
		m_device				=	CreateDevice();
		m_dxManager = new DexianManager();
#endif
	}
	...
	{CPUTimeT(Manager);
		...
		if(bBlendMode)
		{
			for(int i=0;i<RenderMode_Max;i++)
			{
				...
				{
					tech.set_technique((SHADER_HANDLE)i);
					if(SetRenderMode((int)tech.handle, bBlendMode))
						return m_currProg;
					return 0;
				}
	...
	{CPUTimeT(Manager);
		...
		CreateProgramFromMode(RenderMode_ColorBuffer, "default_init_zip/shader/tech_colorbuffer.inl");
		CreateProgramFromMode(RenderMode_TextBuffer, "default_init_zip/shader/tech_textbuffer.inl");
		CreateProgramFromMode(RenderMode_ImageBuffer, "default_init_zip/shader/tech_imagebuffer.inl");
		CreateProgramFromMode(RenderMode_Text, "default_init_zip/shader/tech_text.inl");
		...
		m_uiClipWindow.Set(0, 0, GetBufferWidth(), GetBufferHeight());
		return OnCreate();
	}
...
//	}
	...
	{CPUTimeT(Manager);
		...
		//	AppSleep(nMill);
		for (int i = 0; i < m_edits.size(); i++)
			m_edits[i]->OnTimer(m_dElapsedTime);
#ifndef EDITOR_OS
		...
		m_dxManager->OnFrameMove(this->m_dAppTime, m_dElapsedTime);
...
//#endif
		...
		if(m_bRenderUI)
		{
			...
			m_dxManager->Render(m_dElapsedTime);
			if(GetActiveSDK())
			{
				this->SetUIDialogSize(true,this->m_gameWidth, this->m_gameHeight, 0);
			...
			if(m_bShowFPS)
			{
				...
				if(GetFPS()>30.0f)
					r.setRGBA(0,1,0,1);
				else if(GetFPS()>20.0f)
					r.setRGBA(1, 1, 0, 1);
				else if(GetFPS()>10.0f)
					r.setRGBA(1, .5, 0, 1);
				else
					r.setRGBA(1, 0, 0, 1);
				Rect screen;
				screen.Set(1, 1, GetUIDialogSizeW(), GetUIDialogSizeH());
				//color4 b(0, 0, 0, 1);
				//DrawTexts(buf, screen, 0, 16, 0, &b, DrawTextAlign_CENTER | DrawTextAlign_TOP);
				screen.Set(0, 0, GetUIDialogSizeW(), GetUIDialogSizeH());
				DrawTexts(buf, screen, 0, 16, 0, &r, DrawTextAlign_CENTER | DrawTextAlign_TOP);
			}
		}
	...
	VOID					Manager::Setup2DSpace(float offsetX, float offsetY, float scaleX, float scaleY)
	{
		m_uiOffset.setxy(offsetX / (float)GetBufferWidth(), offsetY / (float)GetBufferHeight());
		m_uiOffsetSave = m_uiOffset;
		m_uiScale.setxy(scaleX, scaleY);
		float fChange = m_uiScale.x - m_uiScaleSave.x;
		RecalcRenderRect();
	}
	...
	VOID					Manager::RecalcRenderRect()
	{
		...
		m_uiClipWindow.left = (float)((float)(-m_uiOffset.x * (float)GetBufferWidth()) / m_uiScale.x);
		m_uiClipWindow.right = (float)((float)(GetBufferWidth() - m_uiOffset.x * (float)GetBufferWidth()) / m_uiScale.x);
		m_uiClipWindow.top = (float)((float)(-m_uiOffset.y * (float)GetBufferHeight()) / m_uiScale.y);
		m_uiClipWindow.bottom = (float)((float)(GetBufferHeight() - m_uiOffset.y * (float)GetBufferHeight()) / m_uiScale.y);
		static _shader_var varUVSO, varR;
		double fY = m_fTan2DFov;
		double fX = fY * (double)this->m_bufferWidth / (double)this->m_bufferHeight;
		m_progMode[RenderMode_ColorBuffer]->SetFloat4(varUVSO, float4(m_uiScale.x, m_uiScale.y, m_uiOffset.x, m_uiOffset.y), "uvSO");
		m_progMode[RenderMode_ColorBuffer]->SetFloat2(varR, float2(fX, fY), "R");
		m_progMode[RenderMode_ImageBuffer]->SetFloat4(varUVSO, float4(m_uiScale.x, m_uiScale.y, m_uiOffset.x, m_uiOffset.y), "uvSO");
		m_progMode[RenderMode_ImageBuffer]->SetFloat2(varR, float2(fX, fY), "R");
		m_progMode[RenderMode_TextBuffer]->SetFloat4(varUVSO, float4(m_uiScale.x, m_uiScale.y, m_uiOffset.x, m_uiOffset.y), "uvSO");
		m_progMode[RenderMode_TextBuffer]->SetFloat2(varR, float2(fX, fY), "R");
	}
	...
	VOID					Manager::PushText(RenderEditText* hr)
	{
		m_edits.push_back(hr);
	}
	VOID					Manager::PopText(RenderEditText* hr)
	{
		for(int i=0;i<m_edits.size();i++)
		{
			if(m_edits[i] == hr)
			{
				m_edits.eraseByIndex(i, 1, true);
				break;
			}
		}
	}
	...
	{CPUTimeT(Manager);
		...
		DexianManager::GetInstance().OnResize();
		m_bSizeChanged = true;
	}
	...
	{CPUTimeNone();
		...
		ret.Set((int)((float)(m_cursorPos[index].x - m_uiOffset.x * (float)GetBufferWidth()) / m_uiScale.x),
			(int)((float)(m_cursorPos[index].y - m_uiOffset.y*(float)GetBufferHeight()) / m_uiScale.y));
	}
	VOID							Manager::GetCursorPos2D(PixelF& ret, unsigned int index)
	{
		CPUTimeNone();
		if (index >= MAX_TOUCH_COUNT)
			index = MAX_TOUCH_COUNT - 1;
		ret.Set((float)((float)(m_cursorPos[index].x - m_uiOffset.x * (float)GetBufferWidth()) / m_uiScale.x),
	...
	{CPUTimeT(Manager);
		...
		if (m_dxManager->OnInputEvent(btn, Pixel(x, y), touchIndex))
		{
		}
		else if(m_uiManager->OnInputEvent(btn, ui, touchIndex))
		{
		}
		else
		{
