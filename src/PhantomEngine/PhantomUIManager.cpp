...
namespace Phantom{
	...
	{CPUTime(UIManager);
		TypeArray<TextureRect>* rects	=	GetTextureRectPtr(textureID);
		if(!rects || !szName)
			return 0;
...
//			UIDialog::ShowTipText(toolTipCtrl);
	...
	TypeArray<TextureRect>*	UIManager::GetTextureRectPtr(int iIndex)										//返回图片指针
	{CPUTime(UIManager);
