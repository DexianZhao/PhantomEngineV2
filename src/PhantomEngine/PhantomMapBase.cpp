...
namespace Phantom{
	...
	{CPUTime(MapBase);
		...
		m_tileCount = size;
		TypeArray<MapGrid> oldMaps = m_mapGrids;
		m_mapGrids.clear();
		m_mapGrids.resize(m_tileCount.cx * m_tileCount.cy);
	...
	{CPUTime(MapBase);
		...
		//m_mapBoxTextures.resize(GetBoxTile().cx * GetBoxTile().cy);
		OnCreateMap();
	}
