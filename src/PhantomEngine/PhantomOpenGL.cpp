...
//			LogInfo("system->error:frame buffer fail(0x%X)", status);
	...
	BOOL	AddElement(TypeArray<OGLElement,short,0>& rets, int& offset, int type, int usage, int usageIndex)
	{
		OGLElement el;
		//
		el.offset		=	offset;
	...
	BOOL	CreateElements(TypeArray<OGLElement,short,0>& rets, VertexElement* elements, int elementCount)
	{
		rets.resize(0);
		int offset			=	0;
	...
	VOID	FillElemenetByFVF(TypeArray<OGLElement,short,0>& rets, INT nFVF)
	{
		rets.clear();
		int	eleIndex	=	0;
		int offset		=	0;
	...
	void		Device_OGL::MakeDrawBuffer(DrawBuffer* buffer, const DrawVInfos& info, DrawMode mode)
	{
		CPUTime(Device_OGL);
		if (info.posList) {
			buffer->PosList.resize(info.m_numVertices);
			memcpy(buffer->PosList.GetPtr(), info.posList, sizeof(float3)*info.m_numVertices);
		}
		else {
			buffer->PosList.resize(0);
		}
		if (info.colorList) {
			buffer->ColorList.resize(info.m_numVertices);
			memcpy(buffer->ColorList.GetPtr(), info.colorList, sizeof(unsigned int)*info.m_numVertices);
		}
		else {
			buffer->ColorList.resize(0);
		}
		if (info.uvList) {
			buffer->UVList.resize(info.m_numVertices);
			memcpy(buffer->UVList.GetPtr(), info.uvList, sizeof(float2)*info.m_numVertices);
		}
		else {
			buffer->UVList.resize(0);
		}
		if (info.NormalList) {
			buffer->NormalList.resize(info.m_numVertices);
			memcpy(buffer->NormalList.GetPtr(), info.normalList, sizeof(float2)*info.m_numVertices);
		}
		else {
			buffer->NormalList.resize(0);
		}
		buffer->NumVertices = info.m_numVertices;
		buffer->Mode = mode;
		buffer->Valid = true;
	}
	void		Device_OGL::MakeDrawBuffer(DrawBuffer* buffer, const DrawVInfos& info, const unsigned short* tList, int numIndices, DrawMode mode)
	{
		MakeDrawBuffer(buffer, info);
		buffer->Indices.resize(numIndices);
		memcpy(buffer->Indices.GetPtr(), tList, sizeof(unsigned short)*numIndices);
	}
	VOID		Device_OGL::Draw(Program* prog, DrawBuffer* buffer)
	{
		BASE_CHECK_NR(glBindBuffer(GL_ARRAY_BUFFER, 0));
		BASE_CHECK_NR(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0));
		//
		prog->SetPointer(prog->m_varPositions, buffer->PosList.size()>0? buffer->PosList.GetPtr():0, "inPosition");
		prog->SetPointer(prog->m_varColors, 4, GL_UNSIGNED_BYTE, GL_TRUE, 0, buffer->ColorList.size()>0 ? buffer->ColorList.GetPtr() : 0, "inColor");
		prog->SetPointer(prog->m_varUVs, buffer->UVList.size()>0 ? buffer->UVList.GetPtr() : 0, "inTexcoord");
		prog->SetPointer(prog->m_varNormals, buffer->NormalList.size()>0 ? buffer->NormalList.GetPtr() : 0, "inNormal");
		//
		if (buffer->Indices.size() > 0) {
			int numIndices = buffer->Indices.size();
			AddRenderTriangle(numIndices / 3);
			int draw = g_ogl->m_DrawMode[buffer->Mode];
			glDrawElements(draw, numIndices, GL_UNSIGNED_SHORT, buffer->Indices.GetPtr());
			if (g_ogl->GetCullMode() == Phantom::CullMode_Double)
			{
			...
			AddRenderTriangle(buffer->NumVertices / 3);
			glDrawArrays(g_ogl->m_DrawMode[buffer->Mode], 0, buffer->NumVertices);
			if (g_ogl->GetCullMode() == Phantom::CullMode_Double)
			{
