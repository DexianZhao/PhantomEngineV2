...
namespace Phantom{
	...
	VOID			QuadBox::SelectObjects(TypeArray<SceneObject*>& objs, const Rect& rc)
	{CPUTimeT(QuadBox);
		if(this->m_nChildObjectCount <= 0)
			return;
		SceneObject* ret = 0;
...
#pragma pack(pop)
	...
	VOID			Scene3D::SelectEvents(TypeArray<ScenePoint*>& rets, const Rect& rc)
	{CPUTimeNone();
	...
	//多选一组模型
	VOID			Scene3D::SelectObjects(TypeArray<SceneObject*>& objs, const Rect& rc)
	{CPUTimeT(Scene3D);
		if(m_pRegionPtr)
	...
	{CPUTimeT(Scene3D);
		...
		m_pRegionPtr		=	&m_regions[nIndex++];
		m_fileName			=	szScene;
		m_staticMeshs.resize(m_objectFilePtr.GetDataCount());
		BuildRegions(m_pRegionPtr, NULL, nIndex, 1, 0, 0, 0);
...
#endif
	VOID					Scene3D::GetObjectByName(const char* szName, TypeArray<SceneObject*>& rets)//返回相关所有对象数组
	{CPUTimeT(Scene3D);
		for(int i=0;i<m_sceneObjects.size();i++)
		{
	...
	BOOL					Scene3D::FindPath(TypeArray<WayPoint2D>& ways, const float3& from, const float3& to)
	{CPUTimeT(Scene3D);
		...
		int index = 0;
		ways.resize(m_astar.getfindPath().getNumPath()+5);
...
#pragma pack(pop)
	VOID					SceneBox::BuildNormals(TypeArray<SceneVertices25d>& vertices, TypeArray<Triangle>& faces, INT nWidth, INT nHeight)
	{CPUTimeNone();
	...
	//VOID					AddAttr(TypeArray<SceneVertices25d>& attrs, SceneVertices25d& v1, SceneVertices25d& v2, SceneVertices25d& v3)
	//{
	//	int old = attrs.size();
	//	attrs.resize(attrs.size()+3);
	//	attrs[old++] = v1;
	//	attrs[old++] = v2;
	//	attrs[old++] = v3;
	//}
	...
	{CPUTimeNone();
		...
		float fGridSize	=	this->m_scenePtr->GetGridSize();
		TypeArray<VertexDiffuse>	s_vertices;
		s_vertices.resize(REGION_VERTEX_COUNT_25d*REGION_VERTEX_COUNT_25d);
		TypeArray<Triangle>	moveEnables;//,moveDisables;
	...
	{CPUTimeNone();
		...
		TypeArray<VertexDiffuse>	s_vertices;
		s_vertices.resize(REGION_VERTEX_COUNT_25d*REGION_VERTEX_COUNT_25d);
		TypeArray<VertexDiffuse>	vertices;
		TypeArray<WORD>	moveEnables;
		int numGrids	=	(REGION_GRID_COUNT_25d + 1);
		//
	...
	VOID					AddAttr(TypeArray<SceneVertices25I>& verts, TypeArray<Triangle>& faces, SceneVertices25d& v1, SceneVertices25d& v2, SceneVertices25d& v3, BOOL bOptimize)
	{
		if(bOptimize){
	...
	{CPUTimeNone();
		...
		float fGridSize	=	this->m_scenePtr->GetGridSize();
		int faceCount		=	0;
		TypeArray<SceneVertices25d>	s_vertices;
		s_vertices.resize(REGION_VERTEX_COUNT_25d*REGION_VERTEX_COUNT_25d);
		TypeArray<GridAttributeT> s_attrs;
		s_attrs.resize(REGION_VERTEX_COUNT_25d*REGION_VERTEX_COUNT_25d);
		...
		TypeArray<Triangle>	facePtr;
		facePtr.resize(REGION_GRID_COUNT_25d*REGION_GRID_COUNT_25d*2);
		for(int y=0;y<REGION_GRID_COUNT_25d;y++)
		{
			for(int x=0;x<REGION_GRID_COUNT_25d;x++)
			{
		...
		TypeArray<SceneVertices25I>	attrs[MAX_LAYER_TEXTURE];
		TypeArray<SceneVertices25I>	attrsA[MAX_LAYER_TEXTURE];
		TypeArray<Triangle>	faceS[MAX_LAYER_TEXTURE];
		TypeArray<Triangle>	faceSA[MAX_LAYER_TEXTURE];
		for(int y=0;y<REGION_GRID_COUNT_25d;y++)
		{
			for(int x=0;x<REGION_GRID_COUNT_25d;x++)
			{
		...
		//TypeArray<WORD> indices;
		TypeArray<SceneVertices25d> tempV;
		for(int i=0;i<MAX_LAYER_TEXTURE;i++)
		{
			if(faceS[i].size() > 0)
			{
