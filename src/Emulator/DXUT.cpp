...
//--------------------------------------------------------------------------------------
HRESULT WINAPI DXUTCreateWindow( HWND hWndParent, const WCHAR* strWindowTitle, HINSTANCE hInstance,
                                 HICON hIcon, HMENU hMenu, int x, int y )
{
    ...
    if( DXUTGetHWNDFocus() == NULL )
    {
        ...
        HWND hWnd = CreateWindow( L"Direct3DWindowClass", strWindowTitle, dwStyle,
                                  x, y, ( rc.right - rc.left ), ( rc.bottom - rc.top ), hWndParent,
                                  hMenu, hInstance, 0 );
        if( hWnd == NULL )
        {
            DWORD dwError = GetLastError();
...
void WINAPI DXUTPause( bool bPauseTime, bool bPauseRendering )
{
    ...
    GetDXUTState().SetRenderingPaused( bPauseRendering );//nPauseRenderingCount > 0 );
    GetDXUTState().SetTimePaused( bPauseTime );//nPauseTimeCount > 0 );
}
...
D3DFORMAT				GetDepthStencilFormat()	//返回当前深度缓冲区格式
{
	DXUTDeviceSettings* pDeviceSettings = GetDXUTState().GetCurrentDeviceSettings();
	return pDeviceSettings->d3d9.pp.AutoDepthStencilFormat;
}
