...
LRESULT CALLBACK MsgProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam, bool* pbNoFurtherProcessing, void* pUserContext )
{
	...
	case WM_LBUTTONDBLCLK:
		nArrow = OnAppInputEvent(2, pos.x, pos.y, 0);
		break;
	case WM_LBUTTONDOWN:
		OnEditBoxSelectNone();
		SetCapture(hWnd);
	...
	case WM_LBUTTONUP:
		ReleaseCapture();
		nArrow = OnAppInputEvent(4, pos.x, pos.y, 0);
		g_bLButtonDown2 = false;
		break;
	case WM_RBUTTONDOWN:
		SetCapture(hWnd);
		nArrow = OnAppInputEvent(2, pos.x, pos.y, 1);
		g_bRButtonDown2 = true;
		break;
	case WM_RBUTTONUP:
		ReleaseCapture();
		nArrow = OnAppInputEvent(4, pos.x, pos.y, 1);
		g_bRButtonDown2 = false;
		break;
	case WM_MBUTTONDOWN:
		SetCapture(hWnd);
		nArrow = OnAppInputEvent(2, pos.x, pos.y, 2);
		g_bMButtonDown2 = true;
		break;
	case WM_MBUTTONUP:
		ReleaseCapture();
		nArrow = OnAppInputEvent(4, pos.x, pos.y, 2);
		g_bMButtonDown2 = false;
		break;
	case WM_MOUSEMOVE:
		{
			if(g_bLButtonDown2)
				nArrow = OnAppInputEvent(1, pos.x, pos.y, 0);
			else if(g_bRButtonDown2)
				nArrow = OnAppInputEvent(1, pos.x, pos.y, 1);
			else if (g_bMButtonDown2)
				nArrow = OnAppInputEvent(1, pos.x, pos.y, 2);
			else
				nArrow = OnAppInputEvent(1, pos.x, pos.y, 3);

	}
		break;
	case WM_MOUSEWHEEL:
		nArrow = OnAppInputEvent(5, (short)HIWORD(wParam), 0, 0);
		break;
	case WM_CLOSE:
	case WM_DESTROY:
		OnAppDestroy();
	...
	case WM_KEYDOWN:
		if(wParam == VK_RETURN)
		{
			nArrow = OnAppInputEvent(6, 0, 0, 0);
		}
		break;
	case WM_SIZE:
		{
