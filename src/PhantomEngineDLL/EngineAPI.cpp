...
PHANTOM_API	VOID	DestroiyDevice()
{
	DXUTDestroyState();
}
EngineAPI::~EngineAPI()
...
BOOL		LoadResData(TypeArray<char>& rets, const wchar_t* szResID, const wchar_t* szResType, HMODULE hModule)
{
...
void		EngineAPI::Init(HWND hWnd)
{
	...
	if (!LoadResData(defaultZip, MAKEINTRESOURCE(IDR_IDR_INIT), L"IDR_INIT", g_hModule))// AfxGetResourceHandle()))
		return;
	//
