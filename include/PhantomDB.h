...
namespace Phantom{
	class	Config;
	struct	PHANTOM_API	ConfigHeader
	{
		Name		name;
		NameT<32>	type_s;
		Text		desc;//说明
...
#pragma pack(pop)
	...
	//	virtual	void		OnRemove(unsigned int id) = 0;
		TypeArray<type>	m_list;
	};
#pragma pack(push, 1)
	struct	PHANTOM_API	StateValue
	{
		char			isNumber;//
		double			value;
		char*			ptr;
	...
	struct	PHANTOM_API	NameState: StateValue
	{
		NameT<31>		name;//最多15个中文文本,最后一个是0
	};
#pragma pack(pop)
	struct	GenuineGUID;
	class PHANTOM_API	GameState: public Base
	{
	public:
		GameState();
		~GameState();
	...
	public:
		...
		NameT<32>						m_name;
		TypeArray<GameState*>			m_childs;//子项
		TypeArray<NameState>			m_states;
		TypeArray<StateValue>			m_arrays;
		//
	...
	void							EncryptData(TypeArray<char>& rets, const char* ptr, int length);
...
#define	MAX_CONFIG_FILE			20//最多支持20个配置文件+一个role_table
	...
	protected:
		TypeArray<Config*>		m_configs;//配置文件列表，没有初始化
		TypeArray<ConfigLine*>	m_lines;//所有行数据,包括固定行和动态行
		//
		friend class				Config;
		int							m_id;
	public:
	...
	PHANTOM_API void						enum_files(const char* path,TypeArray<FileName>& _out, int subFolderLevel, NameT<16>* szFindType,int numFindTypes);
};
