...
struct PHANTOM_API	LuaDefine
{
	Name					name;
	std::string				text;
};

class	PHANTOM_API	CLuaCallback: public Base
{
public:
	ImplementComValue();
...
class PHANTOM_API	LuaAction: public Base
{
public:
	LuaAction();
	~LuaAction();
	...
	double				m_dLastTime;
public:
	TypeArray<ActionVal>	m_vals;
	int					m_currentIndex;
	double				m_dTime;
	Name				m_luaFunc;
};
class PHANTOM_API	CLuaState{
public:
	lua_State*			m_luaState;
};

class PHANTOM_API	CLuaFileMgr: public RenderNodeManager
{
public:
	CLuaFileMgr();
	~CLuaFileMgr();
	...
	VOID				ClearTimeAll(){m_times.resize(0);}
	VOID				BreakTime();
	...
	TypeArray<LuaBindName>	m_luaBindIDs;
...
protected:
	...
	TypeArray<LuaDefine*>	m_defines;//lua宏
	TypeArray<CLuaFile*>	m_files;//lua文件列表
	TypeArray<CLuaFile*>	m_runlist;//运行列表
	TypeArray<LuaCreat>		m_creats;
	TypeArray<LuaTimeData>	m_times;
	int							m_timeID;
};
extern	PHANTOM_API 	CLuaFileMgr		g_luamanager;
}
