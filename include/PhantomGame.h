...
namespace Phantom{
	class Scene3D;
	class	PHANTOM_API GamePower{
	public:
		GamePower();
	public:
		TypeArray<GameObject*>	m_objects;
		int							m_powerID;
		int							m_enemyID;
	...
	class	PHANTOM_API GameMgr: public ConfigMgr{
	public:
		GameMgr();
		virtual ~GameMgr();
	...
	public:
		TypeArray<GameObject*>				m_objects;//游戏对象列表
		TypeArray<GameObject*>				m_inobjects;//可选游戏对象列表
		TypeArray<ScriptBase*>				m_scripts;//脚本列表
		TypeArray<GamePower*>				m_powerList;//势力列表
		TypeArray<FileName>					m_filelist;
		GameID						m_cameraTarget;//跟随镜头的角色
		BOOL						m_cameraFollow;//跟随镜头的角色
		TypeArray<Texture*>					m_loadlist;
		int										m_loadCount;
	protected:
		...
		//
		bool						AddScripts(ScriptBase::eScript type, TypeArray<ScriptBase*>& scripts, const char* config, const char* func, int loadID=-1);
		//
		...
		TypeArray<GameMessage*>		m_frees;//空闲消息数据
		TypeArray<GameMessage*>		m_allocs;//空闲消息数据
		TypeArray<GameMessage*>		m_recvs;//接收的消息数据
		TypeArray<GameMessage*>		m_tempRecvs;//接收的消息数据,当前处理
	...
	PHANTOM_API GameMgr*	GetGameMgr();
};
