...
#define	MAX_LINE_COUNT	256
	...
	class WayPoints;
	class PakDirectory;
	class  PHANTOM_API UIGroup:public Base
	{
	public:
		UIGroup();
		~UIGroup();
	...
	class	 PHANTOM_API DialogAstar: public AstarCallback
	{
	public:
		UIDialog*		m_dlgPtr;
	...
	class  PHANTOM_API UIDialog: public Base
	{
	public:
		ImplementComValue();
		...
		TypeArray<Map2DGridInfo>		m_mapGrids;	//地图格子信息
		TypeArray<Map2DAstarInfo>		m_mapAstarGrids;	//地图格子信息
		...
		TypeArray<RenderData>			m_renders;//子对话框列表,指针根据对话框释放加减
		TypeArray<UIDialog*>			m_childs;//子对话框列表,指针根据对话框释放加减
		Name							m_luaFunc;
		TypeArray<float2>				m_wayPoints;//子对话框列表,指针根据对话框释放加减
		float2							m_wayFrom;
		float2							m_wayDir;
...
//		B2World							m_box2World;
	...
	protected:
		...
		INT								m_nDialogID;
		TypeArray<UIControl*>			m_controls;
		color4							m_dlgColor;
		BOOL							m_bTopMost;
		INT								m_nDlgOrder;
		int								m_nElementID;
