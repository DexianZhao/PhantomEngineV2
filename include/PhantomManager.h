...
#pragma pack(pop)
	...
	class MyMap2D;
	//sdk base
	class PHANTOM_API SDKBase: public Base
	{
	public:
		SDKBase(){}
		virtual	~SDKBase(){}
	...
	class DexianManager;
	//幻影管理器
	class PHANTOM_API Manager: public ThreadProc
	{
	public:
		Manager();
		~Manager();
	...
	public://2D draws, x, y, (z must be 0), lookat z
		...
		//
		VOID						RenderRect2D(DrawBuffer* buffer, const RectF& rc, const color4& c, bool bFill = true);	//渲染2D线条
		VOID						RenderRoundRect2D(DrawBuffer* buffer, const RectF& rc, float Radius, int nMode/*1=lefttop,2=righttop,4=leftbottom,8=rightbottom*/, const color4* c4);	//渲染2D线条
		VOID						RenderLines2D(DrawBuffer* buffer, const RectF& rc, const PixelF* xys, int count, const color4& c, float FillWidth);	//渲染2D线条
		VOID						RenderCircle2D(DrawBuffer* buffer, const PixelF& center, float fRadius, const color4& c, int smooth);//渲染一个圆
		VOID						RenderText2D(DrawBuffer* buffer, RenderText* textPtr, const RectF& rc, const color4& c, const RectF* TextRect = 0);
		void						RenderTexture(DrawBuffer* buffer, Texture* t, const RectF& screen, const RectF& texRc, const color4& c);
		VOID						RenderText2D(DrawBuffer* buffer, RenderEditText* textPtr, const RectF& rc, const color4& c, const RectF* TextRect = 0);
		BOOL						IsRenderVisible(const RectF& rc);
		//
		...
		VOID						DrawRect2D(const RectF& rc, const color4& c, BOOL bFill/*是否填充*/, BOOL bLingXing=false/*渲染菱形*/);	//渲染2D线条
		VOID						DrawRoundRect2D(const RectF& rc, float Radius, int nMode/*1=lefttop,2=righttop,4=leftbottom,8=rightbottom*/, const color4* c4, BOOL bFill/*是否填充*/, BOOL bLingXing = false/*渲染菱形*/);	//渲染2D线条
		VOID						DrawLines2D(const PixelF* xys, int count, const color4& c, BOOL bFill/*是否填充*/, float FillWidth = 1.0f);	//渲染2D线条
		...
		VOID						DrawCircle( const PixelF& center, float fRadius, const color4& c, int smooth, BOOL bFill);//渲染一个圆
		...
		inline	float				GetTan2DFov() { return m_fTan2DFov; }
		//
		...
		VOID						GetCursorPos2D(PixelF& ret, unsigned int index = 0);
		//
	...
	public://base scales
		...
		VOID						Setup2DSpace(float offsetX, float offsetY, float fScaleX, float fScaleY);
	...
	public://render lists
		...
		int							m_textid;
		TypeArray< DrawTextItem > m_textItems;
	...
	protected://enables
		...
		TypeArray<Texture*>		m_loadTextures;	//预载入的图片
		//2D matrix
		double						m_f2DFov;
		double						m_fTan2DFov;
	...
	protected://textures
		...
		TypeArray<Mesh*>		m_meshs;
		TypeArray<Music2D*>		m_backMusics;
		TypeArray<HttpRequest*>	m_https;
		TypeArray<MyMap2D*>		m_map2ds;
		TypeArray<Scene3D*>		m_scene3ds;
		TypeArray<RenderTarget*>	m_RTs;
		...
		VOID						PushText(RenderEditText* hr);
		VOID						PopText(RenderEditText* hr);
	...
	protected://musics
		TypeArray<MultiMusic*>	m_musics;
		//
		TypeArray<float3>		m_tempPosList;
		TypeArray<Triangle>		m_tempFaceList;
		TypeArray<unsigned int>	m_tempColorList;
		TypeArray<RenderText*>	m_texts;
		TypeArray<RenderEditText*>	m_edits;
		struct RenderTextPool{
			RenderText*				text;
			int						count;
		};
		TypeArray<RenderTextPool>	m_textPool;
		//
		VOID						OnRemoveAction(ActionBase* a);
		TypeArray<ActionBase*>	m_actions;
		//
	public:
	...
	protected:
		...
		TypeArray<TIDMessage*>		m_frees;//空闲消息数据
		TypeArray<TIDMessage*>		m_allocs;//空闲消息数据
		TypeArray<TIDMessage*>		m_recvs;//接收的消息数据
		TypeArray<TIDMessage*>		m_tempRecvs;//接收的消息数据,当前处理
		TypeArray<TIDMessage*>		m_trecvs;//接收的消息数据,当前处理
		TypeArray<TIDMessage*>		m_tempTRecvs;//接收的消息数据,当前处理
		void						onMessage(TIDMessage* msg);//处理游戏消息接收
		virtual	VOID				OnThread();
	};

	extern	PHANTOM_API	Manager			g_manager;
	inline	Camera*			GetCameraPtr(){return g_manager.m_camera;}
	extern	PHANTOM_API	BOOL			LoadTexture(TexturePtr& r, char* szTextureFile, const char* szTexturePath, BOOL bGenMipmap, BaseDirectory* basedir = 0);
	extern	PHANTOM_API	VOID			SetEncryptData(unsigned char* crypt, int len);
	extern "C"{
		//PHANTOM_API	const char*		call_lua(const char* type, const char* params, const char* params2);//调用lua内部的函数oncall
	};
};
