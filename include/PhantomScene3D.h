...
namespace Phantom{
	...
	class PHANTOM_API SceneStaticMesh: public RenderBase
	{
	public:
		virtual	void				Render();
	...
	class PHANTOM_API SceneObjectSkill: public MeshCallback
	{
	public:
...
#define	QUAD_BOX_COUNT	4
	...
	public:
		...
		//多选一组模型
		VOID				SelectObjects(TypeArray<SceneObject*>& rets, const Rect& rc);
		//
	...
	protected:
		TypeArray<SceneStaticMesh*>	m_staticObjects;
	...
	class PHANTOM_API SceneBox: public QuadBox
	{
	public:
		SceneBox();
		~SceneBox();
		...
		VOID					BuildNormals(TypeArray<SceneVertices25d>& vertices, TypeArray<Triangle>& faces, INT nWidth, INT nHeight);
#endif
	...
	class PHANTOM_API Scene3DAstar: public AstarCallback
	{
	public:
	...
	class	PHANTOM_API ScenePoint: public Base{
	public:
	...
	PHANTOM_API  BOOL			GetSceneEventName(char* retFunc, char* retFull, int id, const char* name);//根据场景名字进行修订
	PHANTOM_API  int			GetSceneEventCount();


	class PHANTOM_API SceneLight:public Base
	{
	public:
		enum LightType{
			LightType_Omni	=	0,//点光
	...
	class PHANTOM_API SceneEnv: public Base
	{
	public:
		...
		TypeArray<SceneLight*>		m_lights;//灯光列表
	...
	protected:
		...
		int							m_id;//环境ID
		TypeArray<SavePosRot>	m_objects;//环境对象数据类
		Scene3D*					m_scenePtr;
		friend class	Scene3D;
	};
	class PHANTOM_API Scene3D: public Base
	{
	public:
		Scene3D();
		ImplementComValue();
		...
		VOID					GetObjectByName(const char* szName, TypeArray<SceneObject*>& rets);//返回相关所有对象数组
		...
		//
		VOID					SelectObjects(TypeArray<SceneObject*>& rets, const Rect& rc);
		VOID					SelectEvents(TypeArray<ScenePoint*>& rets, const Rect& rc);
		SceneObject*			Intersect(IntersectData* intersect, BOOL bCollisionOnly);	//只点击带碰撞盒的模型
		...
		//
		BOOL					FindPath(TypeArray<WayPoint2D>& ways, const float3& from, const float3& to);
		...
		TypeArray<SceneEnv*>	m_sceneEnvs;//环境设定列表
		...
		TypeArray<SceneGameSkill*>		m_gameSkills;
		TypeArray<SceneObjectSkill*>	m_skills;//技能方案列表,每个技能方案都会被自动载入数据，只要被使用过就会被载入，没有被使用过，第一次使用会被载入
		...
		TypeArray<HitObject>	m_hits;//被击特效播放控件列表
	...
	protected:
		...
		FileName				m_fileName;
		NameT<32>				m_name;
		TypeArray<BindEvent>		m_binds;//由模型属性来设置并生成到m_binds
		TypeArray<SceneMeshFile>	m_staticMeshs;
		TypeArray<SceneObject*>	m_sceneObjects;
