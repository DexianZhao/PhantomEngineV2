...
//	TypeArray<Pixel>			m_tilesIds;	//格子渲染顺序
...
struct	MapUnit_Data
{
	...
	int							type;//自定义类型
	int							layer;//对应层
	TypeArray<MapUnitAction>	actions;
	int							action_move;
	int							action_stand;
};
...
public:
	...
	Pixel						m_pos;//位置
	Map2D_2x1*					m_mapPtr;
	//TypeArray<EventInvoke>	m_invokes;
};
...
public://必须继承的函数
	...
	//
	BOOL							FindPath(TypeArray<WayPoint2D>& ways, const Pixel& mapFrom, const Pixel& mapTo);
	BOOL							GridFromPath(GridIndex& ret, const PathIndex& p);
	VOID							SetPathSize(int w,int h);//设置可移动格子大小
	VOID							RecalcMoveStates();
	Size							m_pathSize;
	TypeArray<PathPoint>		m_pathPoints;//可移动格子设置
...
public:
	...
	//
	TypeArray<MapItemData>		m_projItems;//对象方案列表
	TypeArray<MapGrid_2x1>		m_projGrids;//格子图片方案
	TypeArray<MapUnit_Data*>	m_projUnits;//单位方案列表
...
public:
	...
	TypeArray<MapItem_2x1*>		m_items;
	TypeArray<MapItem_Unit*>	m_units;
protected:
	virtual	VOID					OnCreateMap();//创建地图
	Size							m_gridSize;	//格子大小
	TypeArray<MapTexture>		m_textureMaps;//地图背景
