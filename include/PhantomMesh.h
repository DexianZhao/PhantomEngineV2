...
struct	PHANTOM_API RibbonEmitterData:RibbonEmitterDataOld
{
	int		boneIndex2;
	char	bIsExport;//是否从max导出的，如果是导出的则在max重新导出的时候自动删除
	RibbonEmitterData();
};
struct PHANTOM_API RibbonEmitterData2: RibbonEmitterData
{
	Texture*	texturePtr;
...
class  PHANTOM_API  CParticleEmitter
{
public:
	CParticleEmitter();
	~CParticleEmitter();
...
class	 PHANTOM_API RibbonEmitter
{
public:
	RibbonEmitter()	
	{
...
#pragma pack(pop)
class  PHANTOM_API MeshData: public Base
{
public:
	MeshData();
...
public:
	...
	TypeArray<RibbonEmitterData2*>		m_ribbons;
	TypeArray<ParticleEmitterData2*>		m_particles;
	TypeArray<MeshPoint>			m_points;//位置信息数据
	TypeArray<MeshCamera>			m_cameras;//镜头信息数据
	char							m_bCharacter;
	Name							m_name;
	TypeArray<SubMeshData*>			m_geometrys;
	TypeArray<SubMeshPolygon*>		m_polygons;
	...
	FileName						m_filename;
	//
	TypeArray<MeshActionRange*>		m_actionMap;
	TypeArray<MeshActionData*>		m_actionIO;
	TypeArray<BoneName>				m_boneNameMap;
	int								m_nID;
...
class  PHANTOM_API MeshR: public RenderBase
{
public:
	Mesh*				m_groupPtr;
	virtual	void					drawObject(const matrix4x4* matPtr = 0);
...
class Unit3D;
class  PHANTOM_API MeshCallback: public Base
{
public:
...
class  PHANTOM_API Mesh: public RenderNode
{
public:
	MeshR							m_renderR;
	...
	TypeArray<BindBoneMesh>			m_binds;
	TypeArray<BindBoneDraw>			m_drawMgrs;
	//
	...
	TypeArray<GameID>				m_childs;//子场景数据
