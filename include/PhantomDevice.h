...
namespace Phantom{
	...
	class	DeviceMesh;
#pragma pack(push,1)
	struct	PHANTOM_API ImageBuffer
	{
		enum BPP{
			PixelFormat_RGB = 1,
			PixelFormat_RGBA = 2,
...
#pragma pack(pop)
	...
	class PHANTOM_API Texture: public Base
	{
	public:
		enum ImageFormat{
	...
	PHANTOM_API unsigned char*						ToSmallRGBA(unsigned char* pixels, int width, int height);
//	//
//	class	DynamicMesh
//	{
//	public:
...
//			int							count;		//数量
//		};
//		TypeArray<MeshAttribute>	m_attributes;
...
//		TypeArray<char>				m_vertices;
//		//
//#else
//		TypeArray<float3>			m_posV;
//		TypeArray<float3>			m_normalV;
//		TypeArray<unsigned int>		m_diffuseV;
//		TypeArray<unsigned int>		m_specularV;
//		TypeArray<float2>			m_uvV;
//		TypeArray<float2>			m_uv2V;
//		TypeArray<float2>			m_uv3V;
//		TypeArray<float2>			m_uv4V;
//		TypeArray<float2>			m_uv5V;
//#endif
//		TypeArray<Triangle>			m_triangles;
//		TypeArray<Triangle>			m_trianglesRenders;
//		TypeArray<unsigned char>	m_triangleAttrs;
//	};
		//
	enum CursorModes {
		Cursor_Arrow = 0,
	...
	class	PHANTOM_API DeviceMesh: public Base
	{
	public:
		DeviceMesh();
		ImplementComValue();
	...
	protected:
		virtual	~DeviceMesh();
		TypeArray<char>					m_vertices;	//顶点列表
		TypeArray<unsigned short>		m_indices;	//Indices
		TypeArray<unsigned char>		m_attributes;
		TypeArray<VertexElement>		m_elements;
		//BOOL							m_bHasTBNormal;
	...
	class	PHANTOM_API RenderText: public Base
	{
	public:
		ImplementComValue();
		RenderText();
		...
		virtual const short_t*			GetTextWide() { return 0; }
		virtual int						GetTextWideLength() { return 0; }
		virtual Size					GetTextDrawSize(const short_t* text, int length) { return Size(0, 0); }
		...
		virtual VOID					SetFontStyle(int nStyle/*Style*/);
		...
		const char*						GetText() { return m_text.GetPtr(); }
		void							Redraw();
	protected:
		virtual	VOID					OnResetDevice() = 0;
		virtual	VOID					OnLostDevice() = 0;
	...
	class RenderEditText;
	class RenderEditTextChanged
	{
	public:
		virtual void					OnChanged(RenderEditText* t, void* Param) = 0;
	};
	class PHANTOM_API RenderEditText : public Base
	{
	public:
		ImplementComValue();
		RenderEditText();
		...
		const char*						GetText() { if (TextUTF.size() > 0)return &TextUTF.at(0); return ""; }
		inline	int						GetCursor() { return Cursor; }
		inline	int						GetSelect() { return Select; }
		virtual void					SetEditorMode() = 0;
		//
		void							SetRealRect(const Rect& sz) { RealRect = sz; }//设置编辑区域大小，实际区域
		//
		int								GetFontSize() { return m_nFontSize; }
		int								SetFontStyle() { return m_nFontStyle; }
		//
		inline  BOOL                    IsEmpty() { return (TextUTF.size() == 0); }
		virtual BOOL                    IsInit() { return false; }
		virtual	VOID					SetFontSize(int nSize);//
		enum Style {
			Default = 0,
		...
		//
		void							Redraw();
		RenderEditTextChanged*			Callback;
		void*							CallbackParam;
	public:
		virtual bool					PickCursor(Pixel Pos, int& Result) = 0;
		virtual void					GetDrawTextRegion(TextSectionRegion& Result) = 0;
		virtual void					DrawTextRegion(char* imageRGBA, int imagePitch, const Size& imageSize) = 0;
		virtual void					OnTimer(double dElapsedTime) = 0;
	public:
		enum AlignText {
			AlignText_Left = 0,
			AlignText_Top = 0,
			AlignText_Right = 0x01,
			AlignText_Bottom = 0x02,
			AlignText_Center = 0x04,
			AlignText_VCenter = 0x08,
		};
		int								Align;//AlignText
		Rect							RealRect;//当前窗口位置大小
		EditLimit						Limit;
		double							dMax, dMin;
	protected:
		virtual	VOID					OnResetDevice() = 0;
...
#define	MAX_TEXT_MTRL	5
	class	PHANTOM_API RenderTextFig: public RenderText
	{
	public:
		RenderTextFig();
		~RenderTextFig();
	...
	public:
		...
		virtual	VOID								SetTextTexture(TextureIndex index, RenderEditText* tex) = 0;//渲染文本的时候被调用
	...
	protected:
		TypeArray<char>							m_vsps_directx9;
		TypeArray<char>							m_vsps_opengles_v;
		TypeArray<char>							m_vsps_opengles_p;
		//
	...
	//
	class	PHANTOM_API RenderTarget: public Texture
	{
	public:
		RenderTarget();
	...
	struct	DrawBuffer
	{
		TypeArray<char>				Vertices;
		int							StrideSize;
		int							NumVertices;
		TypeArray<unsigned short>	Indices;
		DrawMode					Mode;
		int							Decl;
		int							NumFace;
		//
		TypeArray<char>				PosList;
		TypeArray<char>				ColorList;
		TypeArray<char>				UVList;
		TypeArray<char>				NormalList;
		BOOL						Valid;
		float						Scale;
		float						Depth;
		float						Rotation;
	...
	public:
		...
		//
		virtual void					MakeDrawBuffer(DrawBuffer* buffer, const DrawVInfos& info, DrawMode mode) = 0;
		virtual void					MakeDrawBuffer(DrawBuffer* buffer, const DrawVInfos& info, const unsigned short* tList, int numIndices, DrawMode mode) = 0;
		virtual	VOID					Draw(Program* prog, DrawBuffer* buffer) = 0;
		//
		...
		virtual	RenderEditText*			CreateRenderEditText() = 0;
		//
	...
	//
	PHANTOM_API VOID					DisableColor(color4& c);
	PHANTOM_API int						FillElemenetByFVF(VertexElement* ele, INT nFVF);
	PHANTOM_API VOID					AddElement(VertexElement* ele, int& eleIndex, int type, int usage, int usageIndex);
	PHANTOM_API Device*					GetDevicePtr();	//唯一的设备
	PHANTOM_API BOOL					IsSizeOK(int s);
	PHANTOM_API BOOL					LoadMesh(StreamBase* stream, MeshPtr& mesh);
	PHANTOM_API VOID					SaveMesh(StreamBase* stream, DeviceMesh* pMesh);
};
