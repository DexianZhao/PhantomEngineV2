...
#pragma pack(push, 1)
	...
	//----------------------------------------------------------------------------------------------------------------
	template <typename T>
	/*STRUCT*/struct	PixelT/*屏幕坐标数据*/
	{
		T		x/*左*/;
		T		y/*上*/;
		/*END_STRUCT*/
		///
		inline	PixelT<T>&	Set(T _x, T _y) { x = _x; y = _y; return *this; }
		inline	void		identity() { x = y = 0; }
		inline	PixelT<T>() { x = y = 0; }
		inline	T	Length() const { return (T)sqrtf(x*x + y*y); }
		PixelT<T>(const PixelT<int>& v)
		{
			x = (T)v.x;	y = (T)v.y;
		}
		PixelT<T>(const PixelT<float>& v)
		{
			x = (T)v.x;	y = (T)v.y;
		}
		inline	PixelT<T> operator = (const PixelT<int>& v)
		{
			x = (T)v.x;	y = (T)v.y;
			return *this;
		}
		inline	PixelT<T> operator = (const PixelT<float>& v)
		{
			x = (T)v.x;	y = (T)v.y;
			return *this;
		}
		PixelT<T>(T _x, T _y) { x = _x; y = _y; }
		//
		inline PixelT<T>& operator	+=(const PixelT<T>& v) { x += v.x; y += v.y; return *this; }
		inline PixelT<T>& operator	-=(const PixelT<T>& v) { x -= v.x; y -= v.y; return *this; }
		inline PixelT<T>& operator	*=(const PixelT<T>& v) { x *= v.x; y *= v.y; return *this; }
		inline PixelT<T>& operator	/=(const PixelT<T>& v) { x /= v.x; y /= v.y; return *this; }
		//inline PixelT<T>& operator	=(const PixelT<T>& v)						{x=v.x;y=v.y;return *this;}
		//
		inline PixelT<T>& operator	+=(T f) { x += f; y += f; return *this; }
		inline PixelT<T>& operator	-=(T f) { x -= f; y -= f; return *this; }
		inline PixelT<T>& operator	*=(T f) { x *= f; y *= f; return *this; }
		inline PixelT<T>& operator	/=(T f) { x /= f; y /= f; return *this; }
		//
		inline PixelT<T>& operator 	++() { x += 1; y += 1; return *this; }
		inline PixelT<T>& operator 	--() { x -= 1; y -= 1; return *this; }
		//
		inline PixelT<T>  operator 	+() const { return *this; }
		inline PixelT<T>  operator 	-() const { return PixelT<T>(-x, -y); }
		//
		inline PixelT<T>  operator 	+(const PixelT<T>& v) const { return PixelT<T>(x + v.x, y + v.y); }
		inline PixelT<T>  operator 	-(const PixelT<T>& v) const { return PixelT<T>(x - v.x, y - v.y); }
		inline PixelT<T>  operator 	*(const PixelT<T>& v) const { return PixelT<T>(x*v.x, y*v.y); }
		inline PixelT<T>  operator 	/(const PixelT<T>& v) const { return PixelT<T>(x / v.x, y / v.y); }
		//
		inline PixelT<T>  operator 	+(T f) const { return PixelT<T>(x + f, y + f); }
		inline PixelT<T>  operator 	-(T f) const { return PixelT<T>(x - f, y - f); }
		inline PixelT<T>  operator 	*(T f) const { return PixelT<T>(x*f, y*f); }
		inline PixelT<T>  operator 	/(T f) const { float fInv = 1.0f / (float)f; return PixelT<T>((T)((float)x*fInv), (T)((float)y*fInv)); }
		//
		inline bool operator 		==(const PixelT<T>& v) const { return (x == v.x&&y == v.y); }
		inline bool operator 		!=(const PixelT<T>& v) const { return (x != v.x || y != v.y); }
		inline bool operator 		==(T f) const { return (x == f&&y == f); }
		inline void					normalize() { float r = Length(); if (r>0.0001f) { r = 1.0f / r; x *= r; y *= r; } }
		inline PixelT<T>			cross()	 const {
			return PixelT<T>(y, -x);
		}
		//
		inline bool operator		!=(T f) const { return (x != f || y != f); }
	};
	typedef PixelT<int>		Pixel;
	typedef PixelT<float>	PixelF;
	typedef PixelT<double>	PixelD;
	template <typename T>
	struct	SizeT
	{
		T		cx;
		T		cy;
		///
		inline	SizeT<T>&	Set(T _x, T _y) { cx = _x; cy = _y; return *this; }
		inline	void		identity() { cx = cy = 0; }
		inline	SizeT<T>() { cx = cy = 0; }
		SizeT<T>(T _x, T _y) { cx = _x; cy = _y; }
		SizeT<T>(const SizeT<int>& v)
		{
			cx = (T)v.cx;	cy = (T)v.cy;
		}
		SizeT<T>(const SizeT<float>& v)
		{
			cx = (T)v.cx;	cy = (T)v.cy;
		}
		inline	SizeT<T> operator = (const SizeT<int>& v)
		{
			cx = (T)v.cx;	cy = (T)v.cy;
			return *this;
		}
		inline	SizeT<T> operator = (const SizeT<float>& v)
		{
			cx = (T)v.cx;	cy = (T)v.cy;
			return *this;
		}
		//
		inline SizeT<T>& operator	+=(const SizeT<T>& v) { cx += v.cx; cy += v.cy; return *this; }
		inline SizeT<T>& operator	-=(const SizeT<T>& v) { cx -= v.cx; cy -= v.cy; return *this; }
		inline SizeT<T>& operator	*=(const SizeT<T>& v) { cx *= v.cx; cy *= v.cy; return *this; }
		inline SizeT<T>& operator	/=(const SizeT<T>& v) { cx /= v.cx; cy /= v.cy; return *this; }
		//inline SizeT<T>& operator	=(const SizeT<T>& v) { cx = v.cx; cy = v.cy; return *this; }
		//
		inline SizeT<T>& operator	+=(T f) { cx += f; cy += f; return *this; }
		inline SizeT<T>& operator	-=(T f) { cx -= f; cy -= f; return *this; }
		inline SizeT<T>& operator	*=(T f) { cx *= f; cy *= f; return *this; }
		inline SizeT<T>& operator	/=(T f) { cx /= f; cy /= f; return *this; }
		//
		inline SizeT<T>& operator 	++() { cx += 1; cy += 1; return *this; }
		inline SizeT<T>& operator 	--() { cx -= 1; cy -= 1; return *this; }
		//
		inline SizeT<T>  operator 	+() const { return *this; }
		inline SizeT<T>  operator 	-() const { return Size(-cx, -cy); }
		//
		inline SizeT<T>  operator 	+(const SizeT<T>& v) const { return SizeT<T>(cx + v.cx, cy + v.cy); }
		inline SizeT<T>  operator 	-(const SizeT<T>& v) const { return SizeT<T>(cx - v.cx, cy - v.cy); }
		inline SizeT<T>  operator 	*(const SizeT<T>& v) const { return SizeT<T>(cx*v.cx, cy*v.cy); }
		inline SizeT<T>  operator 	/(const SizeT<T>& v) const { return SizeT<T>(cx / v.cx, cy / v.cy); }
		//
		inline SizeT<T>  operator 	+(T f) const { return SizeT<T>(cx + f, cy + f); }
		inline SizeT<T>  operator 	-(T f) const { return SizeT<T>(cx - f, cy - f); }
		inline SizeT<T>  operator 	*(T f) const { return SizeT<T>(cx*f, cy*f); }
		inline SizeT<T>  operator 	/(T f) const { float fInv = 1.0f / (float)f; return SizeT<T>((T)((float)cx*fInv), (T)((float)cy*fInv)); }
		//
		inline BOOL operator 		==(const SizeT<T>& v) const { return (cx == v.cx&&cy == v.cy); }
		inline BOOL operator 		!=(const SizeT<T>& v) const { return (cx != v.cx || cy != v.cy); }
		inline BOOL operator 		==(T f) const { return (cx == f&&cy == f); }
		//
		inline BOOL operator		!=(T f) const { return (cx != f || cy != f); }
	};
	...
	/*STRUCT*/struct	RectT/*矩形数据*/
	{
		...
		inline	void		SetBeginPoint(int _left, int _top) {
			int w = right - left;
			int h = bottom - top;
			left = _left;
			top = _top;
		...
		inline	void		Width(const T t) { right = left + t; }
		inline	void		Height(const T t) { bottom = top + t; }
		inline	RectT<T>&	Set(T l, T t, T r, T b) { left = l;	top = t;	right = r;	bottom = b;	return *this; }
		...
		inline	bool		isPtIn(T x, T y) const { return ((left < x && right > x) && (top < y && bottom > y)); }
		inline	bool		isPtIn(const PixelT<T>& p) const { return ((left < p.x && right > p.x) && (top < p.y && bottom > p.y)); }
		inline	void		clip(T minLeft, T minTop, T maxRight, T maxBottom)
		{
			//
			left = getmax(minLeft, left); top = getmax(minTop, top); bottom = getmin(maxBottom, bottom); right = getmin(maxRight, right);
		}
		inline	RectT<T>() { left = right = top = bottom = 0; }
		RectT<T>(const RectT<int>& v)
		{
			left = (T)v.left;	top = (T)v.top;	right = (T)v.right;	bottom = (T)v.bottom;
		}
		RectT<T>(const RectT<float>& v)
		{
			left = (T)v.left;	top = (T)v.top;	right = (T)v.right;	bottom = (T)v.bottom;
		}
		inline	RectT<T> operator = (const RectT<int>& v)
		{
			left = (T)v.left;	top = (T)v.top;	right = (T)v.right;	bottom = (T)v.bottom;
			return *this;
		}
		inline	RectT<T> operator = (const RectT<float>& v)
		{
			left = (T)v.left;	top = (T)v.top;	right = (T)v.right;	bottom = (T)v.bottom;
			return *this;
		}
		RectT<T>(T l, T t, T r, T b) { left = l;	top = t;	right = r;	bottom = b; }
		RectT<T>(PixelT<T> pos, T w, T h)
		{
			left = pos.x;
			top = pos.y;
			right = pos.x + w;
		...
		//inline	RectT<T>&	operator	=	(const RectT<T>& rc)				{	left=rc.left;	top=rc.top;	right=rc.right;	bottom=rc.bottom;	return *this;	}
		//
		inline RectT<T>  operator 	+(const RectT<T>& v) const { return RectT<T>(left + v.left, top + v.top, right + v.right, bottom + v.bottom); }
		inline RectT<T>  operator 	-(const RectT<T>& v) const { return RectT<T>(left - v.left, top - v.top, right - v.right, bottom - v.bottom); }
		inline RectT<T>  operator 	*(const RectT<T>& v) const { return RectT<T>(left * v.left, top * v.top, right * v.right, bottom * v.bottom); }
		inline RectT<T>  operator 	/(const RectT<T>& v) const { return RectT<T>(v.left != 0 ? (left / v.left) : 0, v.top != 0 ? (top / v.top) : 0, v.right != 0 ? (right / v.right) : 0, v.bottom != 0 ? (bottom / v.bottom) : 0); }
		inline bool	  operator 	==(const RectT<T>& v) const { return (left == v.left && top == v.top && right == v.right && bottom == v.bottom); }
		//
		inline	T			MiddleX() { return (left + (right - left) / 2); }
		inline	T			MiddleY() { return (top + (bottom - top) / 2); }
	};
	...
	/*STRUCT*/struct float2/*二维向量*/
	{
		...
		inline float2  operator 	-(float f) const				{return float2(x-f,y-f);}
		inline float2  operator 	*(float f) const				{return float2(x*f,y*f);}
		inline float2  operator 	/(float f) const				{float fInv = 1.0f / f;return float2(x*fInv,y*fInv);}
		//
		...
		inline float2				maxv(const float2& v)	const	{return float2(getmax(v.x,x),getmax(v.y,y));}
		inline float2				minv(const float2& v)	const	{return float2(getmin(v.x,x),getmin(v.y,y));}
		inline float2				maxf(float f)			const	{return float2(getmax(f,x),getmax(f,y));}
		inline float2				minf(float f)			const	{return float2(getmin(f,x),getmin(f,y));}
		inline float2				absv()					const	{return float2(fabsf(x),fabsf(y));}
	...
	//----------------------------------------------------------------------------------------------------------------
	//三维平面数据
	/*STRUCT*/struct	PHANTOM_API	Plane/*平面*/
	{
		float a;
		float b;
		float c;
	...
	struct TextSection
	{
		...
		char	text[Length];
		bool	operator == (const TextSection& t) {
			return (stricmp(text, t.text) == 0);
		}
	};
	PHANTOM_API	int	SplitText(const char* szText, TextSection* secs, int secCount, char symbol, bool bRemoveSpace = true);
	...
	//----------------------------------------------------------------------------------------------------------------
	//颜色数据
	/*STRUCT*/struct PHANTOM_API color4/*颜色*/
	{
		float r;
		float g;
		float b;
		...
		//从ARGB值转换成颜色数据
		static color4			fromRGBA(unsigned char R, unsigned char G, unsigned char B, unsigned char A);
		//返回GDI的RGB值
	...
	//----------------------------------------------------------------------------------------------------------------
	//三维矩阵数据
	/*STRUCT*/struct PHANTOM_API matrix4x4/*矩阵*/
	{
		union{
			struct{
				float		_11;
...
#pragma pack(pop)
...
#ifdef WIN32_OS
	typedef LPCSTR	SHADER_HANDLE;
#else
	typedef int		SHADER_HANDLE;
#endif
	...
	struct	_shader_var
	{
		...
		void					set_handle(SHADER_HANDLE h)		{handle = h;	itype = shader_find_ok;}
		void					set_technique(SHADER_HANDLE h)		{handle = h;	itype = shader_find_technique;}
		void					reset_handle()					{handle = 0;	itype = 0;}
		//
	};

	inline	float		RandomF(float fMin, float fMax)					{return (fMin + (fMax - fMin) * ((float)rand() / RAND_MAX));}
	PHANTOM_API 	float		getRotateZ(const float3& _dir);
	PHANTOM_API 	float		clampAngle(float fAngle);
	class	IntersectData;
	PHANTOM_API 	BOOL		IntersectTriangle(PIntersectData* pOutPick, const float3& vOrig, const float3& vDir, const float3& v0, const float3& v1, const float3& v2, const matrix4x4* pWorld, bool bDoubleSide = false);
	PHANTOM_API 	BOOL		IntersectTriangle(const Pixel& cursor, const Pixel& v0, const Pixel& v1, const Pixel& v2);
	PHANTOM_API 	BOOL		IntersectTriangleList(PIntersectData* pOutPick, const float3& vOrig, const float3& vDir, const void* _verts, const void* pIndices, int numFace, const matrix4x4* pWorld, int strideSize, int cullMode = CullMode_Double);
	PHANTOM_API 	float		LineCrossPoint(float3* pOutCross, const float3& orig, const float3& dir, const float3& point);
	PHANTOM_API 	float		LineInPointY(float2* vOutCross, const float2& orig, const float2& dir, const float2& point);
	PHANTOM_API 	float		LineInPointX(float2* vOutCross, const float2& orig, const float2& dir, const float2& point);
	PHANTOM_API 	BOOL		PickBoxLinear(const float2& orig, const float2& dir, const float2& vMin, const float2& vMax, float2* retPick);
	PHANTOM_API 	BOOL		IntersectBox(const float3& vOrig, const float3& vDir, const float3& vMin, const float3& vMax, const matrix4x4* pWorld);
	PHANTOM_API 	BOOL		IsSphereInAABox(const AABox& parent, const PSphereData& child);
	PHANTOM_API 	BOOL		IsPointInAABox(const AABox& parent, const float3& child);
	PHANTOM_API 	BOOL		IsFullClipAABox(const AABox& parent, const AABox& child);
	PHANTOM_API 	BOOL		IsAABoxInAABox(const AABox& parent, const AABox& child);
	//
	PHANTOM_API 	float2		HermiteCurve(const float2& p1,const float2& p2,const float2& t1,const float2& t2, float fLerp);
	PHANTOM_API 	float2		LineCurve(const float2& p1,const float2& p2, float fLerp);
	PHANTOM_API 	float		clampDegress(float fDegress);//将弧度限制范围在 -PI ~ PI 之间
	//----------------------------------------------------------------------------------------------------------------
	PHANTOM_API 	void		IntersectPlane(const float3& point, const float3& normal, const float3& from, const float3& to, float3& pickPoint);
	//----------------------------------------------------------------------------------------------------------------
	PHANTOM_API float2*				vec2Normalize( float2 *pOut, const float2 *pV );
	PHANTOM_API float2*				vec2Hermite( float2 *pOut, const float2 *pV1, const float2 *pT1,const float2 *pV2, const float2 *pT2, float s );// Hermite interpolation between position V1, tangent T1 (when s == 0// and position V2, tangent T2 (when s == 1).
	PHANTOM_API float2*				vec2CatmullRom( float2 *pOut, const float2 *pV0, const float2 *pV1,const float2 *pV2, const float2 *pV3, float s );// CatmullRom interpolation between V1 (when s == 0) and V2 (when s == 1)
	PHANTOM_API float2*				vec2BaryCentric( float2 *pOut, const float2 *pV1, const float2 *pV2,const float2 *pV3, float f, float g);// Barycentric coordinates.  V1 + f(V2-V1) + g(V3-V1)
	PHANTOM_API float4*				vec2Transform( float4 *pOut, const float2 *pV, const matrix4x4 *pM );// Transform (x, y, 0, 1) by matrix.
	PHANTOM_API float2*				vec2TransformCoord( float2 *pOut, const float2 *pV, const matrix4x4 *pM );// Transform (x, y, 0, 1) by matrix, project result back into w=1.
	PHANTOM_API float2*				vec2TransformNormal( float2 *pOut, const float2 *pV, const matrix4x4 *pM );// Transform (x, y, 0, 0) by matrix.
	PHANTOM_API float4*				vec2TransformArray( float4 *pOut, unsigned int OutStride, const float2 *pV, unsigned int VStride, const matrix4x4 *pM, unsigned int n);// Transform Array (x, y, 0, 1) by matrix.
	PHANTOM_API float2*				vec2TransformCoordArray( float2 *pOut, unsigned int OutStride, const float2 *pV, unsigned int VStride, const matrix4x4 *pM, unsigned int n );// Transform Array (x, y, 0, 1) by matrix, project result back into w=1.
	PHANTOM_API float2*				vec2TransformNormalArray( float2 *pOut, unsigned int OutStride, const float2 *pV, unsigned int VStride, const matrix4x4 *pM, unsigned int n );// Transform Array (x, y, 0, 0) by matrix.
	...
	//----------------------------------------------------------------------------------------------------------------
	//float3
	PHANTOM_API float3*				vec3Normalize( float3 *pOut, const float3 *pV );
	PHANTOM_API float3*				vec3Hermite( float3 *pOut, const float3 *pV1, const float3 *pT1,const float3 *pV2, const float3 *pT2, float s );// Hermite interpolation between position V1, tangent T1 (when s == 0)// and position V2, tangent T2 (when s == 1).
	PHANTOM_API float3*				vec3CatmullRom( float3 *pOut, const float3 *pV0, const float3 *pV1,const float3 *pV2, const float3 *pV3, float s );// CatmullRom interpolation between V1 (when s == 0) and V2 (when s == 1)
	PHANTOM_API float3*				vec3BaryCentric( float3 *pOut, const float3 *pV1, const float3 *pV2,const float3 *pV3, float f, float g);// Barycentric coordinates.  V1 + f(V2-V1) + g(V3-V1)
	PHANTOM_API float3*				vec3Transform( float3 *pOut, const float3 *pV, const matrix4x4 *pM );// Transform (x, y, z, 1) by matrix.
	PHANTOM_API float3*				vec3TransformCoord( float3 *pOut, const float3 *pV, const matrix4x4 *pM );// Transform (x, y, z, 1) by matrix, project result back into w=1.
	PHANTOM_API float3*				vec3TransformNormal( float3 *pOut, const float3 *pV, const matrix4x4 *pM );// Transform (x, y, z, 0) by matrix.  If you transforming a normal by a // non-affine matrix, the matrix you pass to this function should be the // transpose of the inverse of the matrix you would use to transform a coord.
	PHANTOM_API float4*				vec3TransformArray( float4 *pOut, unsigned int OutStride, const float3 *pV, unsigned int VStride, const matrix4x4 *pM, unsigned int n );// Transform Array (x, y, z, 1) by matrix. 
	PHANTOM_API float3*				vec3TransformCoordArray( float3 *pOut, unsigned int OutStride, const float3 *pV, unsigned int VStride, const matrix4x4 *pM, unsigned int n );// Transform Array (x, y, z, 1) by matrix, project result back into w=1.
	PHANTOM_API float3*				vec3TransformNormalArray( float3 *pOut, unsigned int OutStride, const float3 *pV, unsigned int VStride, const matrix4x4 *pM, unsigned int n );// Transform (x, y, z, 0) by matrix.  If you transforming a normal by a // non-affine matrix, the matrix you pass to this function should be the // transpose of the inverse of the matrix you would use to transform a coord.
	PHANTOM_API float				vec3Length( const float3 *pV );
	PHANTOM_API float				vec3LengthSq( const float3 *pV );
	PHANTOM_API float				vec3Dot( const float3 *pV1, const float3 *pV2 );
	PHANTOM_API float3*				vec3Cross( float3 *pOut, const float3 *pV1, const float3 *pV2 );
	PHANTOM_API float3*				vec3Add( float3 *pOut, const float3 *pV1, const float3 *pV2 );
	PHANTOM_API float3*				vec3Subtract( float3 *pOut, const float3 *pV1, const float3 *pV2 );
	PHANTOM_API float3*				vec3Minimize( float3 *pOut, const float3 *pV1, const float3 *pV2 );
	PHANTOM_API float3*				vec3Maximize( float3 *pOut, const float3 *pV1, const float3 *pV2 );
	PHANTOM_API float3*				vec3Scale( float3 *pOut, const float3 *pV, float s);
	PHANTOM_API float3*				vec3Lerp( float3 *pOut, const float3 *pV1, const float3 *pV2,float s );
	//----------------------------------------------------------------------------------------------------------------
	//float4
	PHANTOM_API float4*				vec4Cross( float4 *pOut, const float4 *pV1, const float4 *pV2,const float4 *pV3);// Cross-product in 4 dimensions.
	PHANTOM_API float4*				vec4Normalize( float4 *pOut, const float4 *pV );
	PHANTOM_API float4*				vec4Hermite( float4 *pOut, const float4 *pV1, const float4 *pT1,const float4 *pV2, const float4 *pT2, float s );// Hermite interpolation between position V1, tangent T1 (when s == 0)// and position V2, tangent T2 (when s == 1).
	PHANTOM_API float4*				vec4CatmullRom( float4 *pOut, const float4 *pV0, const float4 *pV1,const float4 *pV2, const float4 *pV3, float s );// CatmullRom interpolation between V1 (when s == 0) and V2 (when s == 1)
	PHANTOM_API float4*				vec4BaryCentric( float4 *pOut, const float4 *pV1, const float4 *pV2,const float4 *pV3, float f, float g);// Barycentric coordinates.  V1 + f(V2-V1) + g(V3-V1)
	PHANTOM_API float4*				vec4Transform( float4 *pOut, const float4 *pV, const matrix4x4 *pM );// Transform vector by matrix.
	PHANTOM_API float4*				vec4TransformArray( float4 *pOut, unsigned int OutStride, const float4 *pV, unsigned int VStride, const matrix4x4 *pM, unsigned int n );// Transform vector array by matrix.
	//----------------------------------------------------------------------------------------------------------------
	//quaternion
	PHANTOM_API void				qtnFromDirection(quaternion& qtn,float3& vFrom, float3 &vTo);
	PHANTOM_API void				qtnToAxisAngle( const quaternion *pQ, float3 *pAxis, float *pAngle );
	PHANTOM_API quaternion* 		qtnRotationMatrix( quaternion *pOut, const matrix4x4 *pM);// Build a quaternion from a rotation matrix.
	PHANTOM_API quaternion* 		qtnRotationAxis( quaternion *pOut, const float3 *pV, float Angle );// Rotation about arbitrary axis.
	PHANTOM_API quaternion* 		qtnRotationYawPitchRoll( quaternion *pOut, float Yaw, float Pitch, float Roll );// Yaw around the Y axis, a pitch around the X axis,// and a roll around the Z axis.
	PHANTOM_API quaternion* 		qtnMultiply( quaternion *pOut, const quaternion *pQ1,const quaternion *pQ2 );// Quaternion multiplication.  The result represents the rotation Q2// followed by the rotation Q1.  (Out = Q2 * Q1)
	PHANTOM_API quaternion* 		qtnNormalize( quaternion *pOut, const quaternion *pQ );// Conjugate and re-norm
	PHANTOM_API quaternion* 		qtnInverse( quaternion *pOut, const quaternion *pQ );// Expects unit quaternions.// if q = (cos(theta), sin(theta) * v); ln(q) = (0, theta * v)
	PHANTOM_API quaternion* 		qtnLn( quaternion *pOut, const quaternion *pQ );
	PHANTOM_API quaternion* 		qtnExp( quaternion *pOut, const quaternion *pQ );// Expects pure quaternions. (w == 0)  w is ignored in calculation.// if q = (0, theta * v); exp(q) = (cos(theta), sin(theta) * v)
	PHANTOM_API quaternion* 		qtnSlerp( quaternion *pOut, const quaternion *pQ1,const quaternion *pQ2, float t );// Spherical linear interpolation between Q1 (t == 0) and Q2 (t == 1).// Expects unit quaternions.
	PHANTOM_API quaternion* 		qtnSquad( quaternion *pOut, const quaternion *pQ1,const quaternion *pA, const quaternion *pB,const quaternion *pC, float t );// Spherical quadrangle interpolation.// Slerp(Slerp(Q1, C, t), Slerp(A, B, t), 2t(1-t))
	PHANTOM_API quaternion*			qtnBaryCentric( quaternion *pOut, const quaternion *pQ1,const quaternion *pQ2, const quaternion *pQ3,float f, float g );// Barycentric interpolation.// Slerp(Slerp(Q1, Q2, f+g), Slerp(Q1, Q3, f+g), g/(f+g))
	//----------------------------------------------------------------------------------------------------------------
	//Plane
	PHANTOM_API Plane*				planeNormalize( Plane *pOut, const Plane *pP);//点法创建平面
	PHANTOM_API float3*				planeIntersectLine( float3 *pOut, const Plane *pP, const float3 *pV1,const float3 *pV2);// Find the intersection between a plane and a line.  If the line is// parallel to the plane, NULL is returned.
	PHANTOM_API Plane*				planeFromPointNormal( Plane *pOut, const float3 *pPoint, const float3 *pNormal);//根据点和法线创建平面
	PHANTOM_API Plane*				planeFromPoints( Plane *pOut, const float3 *pV1, const float3 *pV2,const float3 *pV3);//根据三点创建平面
	PHANTOM_API Plane*				planeTransform( Plane *pOut, const Plane *pP, const matrix4x4 *pM );// Transform a plane by a matrix.  The vector (a,b,c) must be normal.// M should be the inverse transpose of the transformation desired.
	PHANTOM_API Plane*				planeTransformArray( Plane *pOut, unsigned int OutStride, const Plane *pP, unsigned int PStride, const matrix4x4 *pM, unsigned int n );// Transform an array of planes by a matrix.  The vectors (a,b,c) must be normal.// M should be the inverse transpose of the transformation desired.
	PHANTOM_API float				planeDot(const Plane* p1, const float4* p2);// ax + by + cz + dw平面方程与4D向量的点乘
	PHANTOM_API float				planeDotCoord(const Plane* p1, const float3* p2);// ax + by + cz + d平面与3D向量的点乘,w=1
	PHANTOM_API float				planeDotNormal(const Plane* p1, const float3* p2);// ax + by + cz平面与3D向量的点乘,w=0
	PHANTOM_API Plane*				planeScale(Plane* pOut, const Plane* p1, float s);
	//----------------------------------------------------------------------------------------------------------------
	//color4
	PHANTOM_API color4*				colorAdjustSaturation(color4 *pOut, const color4 *pC, float s);// Interpolate r,g,b between desaturated color and color.// DesaturatedColor + s(Color - DesaturatedColor)
	PHANTOM_API color4*				colorAdjustContrast(color4 *pOut, const color4 *pC, float c);// Interpolate r,g,b between 50% grey and color.  Grey + s(Color - Grey)
	//----------------------------------------------------------------------------------------------------------------
	//matrix4x4
	PHANTOM_API matrix4x4*			matrixLerp(matrix4x4* pOut, const matrix4x4* pFrom, const matrix4x4* pTo, float fLerp);
	PHANTOM_API matrix4x4*			matrixTransform(matrix4x4* bzOut, const float3* pos, const quaternion* rot, const float3* scale);	//转变,根据参数
	PHANTOM_API void				getRSP(quaternion& outQtn,float3& outScale,float3& outPosition,const matrix4x4& matIn_);
	PHANTOM_API void				getMatrixQuaternion(quaternion& _out,const matrix4x4& _matIn);
	PHANTOM_API void				getMatrixScale(float3& outScale,const matrix4x4& _matIn);
	inline	void		matrixIdentity(matrix4x4* m){m->identity();}
	//
	PHANTOM_API BOOL				matrixDecompose( float3 *pOutScale, quaternion *pOutRotation, float3 *pOutTranslation, const matrix4x4 *pM );
	PHANTOM_API matrix4x4*			matrixTranspose( matrix4x4 *pOut, const matrix4x4 *pM );
	PHANTOM_API matrix4x4* 			matrixMultiply( matrix4x4 *pOut, const matrix4x4 *pM1, const matrix4x4 *pM2 );// Matrix multiplication.  The result represents the transformation M2// followed by the transformation M1.  (Out = M1 * M2)
	PHANTOM_API matrix4x4* 			matrixMultiplyTranspose( matrix4x4 *pOut, const matrix4x4 *pM1, const matrix4x4 *pM2 );// Matrix multiplication, followed by a transpose. (Out = T(M1 * M2))
	PHANTOM_API matrix4x4* 			matrixInverse( matrix4x4 *pOut, float *pDeterminant, const matrix4x4 *pM );// Calculate inverse of matrix.  Inversion my fail, in which case NULL will// be returned.  The determinant of pM is also returned it pfDeterminant// is non-NULL.
	PHANTOM_API matrix4x4* 			matrixScaling( matrix4x4 *pOut, float sx, float sy, float sz );// Build a matrix which scales by (sx, sy, sz)
	PHANTOM_API matrix4x4* 			matrixTranslation( matrix4x4 *pOut, float x, float y, float z );// Build a matrix which translates by (x, y, z)
	PHANTOM_API matrix4x4* 			matrixRotationX( matrix4x4 *pOut, float Angle );// Build a matrix which rotates around the X axis
	PHANTOM_API matrix4x4* 			matrixRotationY( matrix4x4 *pOut, float Angle );// Build a matrix which rotates around the Y axis
	PHANTOM_API matrix4x4* 			matrixRotationZ( matrix4x4 *pOut, float Angle );// Build a matrix which rotates around the Z axis
	PHANTOM_API matrix4x4* 			matrixRotationAxis( matrix4x4 *pOut, const float3 *pV, float Angle );// Build a matrix which rotates around an arbitrary axis
	PHANTOM_API matrix4x4* 			matrixRotationQuaternion( matrix4x4 *pOut, const quaternion *pQ);// Build a matrix from a quaternion
	PHANTOM_API matrix4x4* 			matrixRotationYawPitchRoll( matrix4x4 *pOut, float Yaw, float Pitch, float Roll );// Yaw around the Y axis, a pitch around the X axis,// and a roll around the Z axis.
	PHANTOM_API matrix4x4* 			matrixTransformation( matrix4x4 *pOut, const float3 *pScalingCenter,const quaternion *pScalingRotation, const float3 *pScaling,const float3 *pRotationCenter, const quaternion *pRotation,const float3 *pTranslation);// Build transformation matrix.  NULL arguments are treated as identity.// Mout = Msc-1 * Msr-1 * Ms * Msr * Msc * Mrc-1 * Mr * Mrc * Mt
	PHANTOM_API matrix4x4* 			matrixTransformation2D( matrix4x4 *pOut, const float2* pScalingCenter, float ScalingRotation, const float2* pScaling, const float2* pRotationCenter, float Rotation, const float2* pTranslation);// Build 2D transformation matrix in XY plane.  NULL arguments are treated as identity.// Mout = Msc-1 * Msr-1 * Ms * Msr * Msc * Mrc-1 * Mr * Mrc * Mt
	PHANTOM_API matrix4x4* 			matrixAffineTransformation( matrix4x4 *pOut, float Scaling, const float3 *pRotationCenter,const quaternion *pRotation, const float3 *pTranslation);// Build affine transformation matrix.  NULL arguments are treated as identity.// Mout = Ms * Mrc-1 * Mr * Mrc * Mt
	PHANTOM_API matrix4x4* 			matrixAffineTransformation2D( matrix4x4 *pOut, float Scaling, const float2* pRotationCenter, float Rotation, const float2* pTranslation);// Build 2D affine transformation matrix in XY plane.  NULL arguments are treated as identity.// Mout = Ms * Mrc-1 * Mr * Mrc * Mt
	PHANTOM_API matrix4x4* 			matrixLookAtRH( matrix4x4 *pOut, const float3 *pEye, const float3 *pAt,const float3 *pUp );// Build a lookat matrix. (right-handed)
	PHANTOM_API matrix4x4* 			matrixLookAtLH( matrix4x4 *pOut, const float3 *pEye, const float3 *pAt,const float3 *pUp );// Build a lookat matrix. (left-handed)
	PHANTOM_API matrix4x4* 			matrixPerspectiveRH( matrix4x4 *pOut, float w, float h, float zn, float zf );// Build a perspective projection matrix. (right-handed)
	PHANTOM_API matrix4x4* 			matrixPerspectiveLH( matrix4x4 *pOut, float w, float h, float zn, float zf );// Build a perspective projection matrix. (left-handed)
	PHANTOM_API matrix4x4* 			matrixPerspectiveFovRH( matrix4x4 *pOut, float fovy, float Aspect, float zn, float zf );// Build a perspective projection matrix. (right-handed)
	PHANTOM_API matrix4x4* 			matrixPerspectiveFovLH( matrix4x4 *pOut, float fovy, float Aspect, float zn, float zf );// Build a perspective projection matrix. (left-handed)
	PHANTOM_API matrix4x4* 			matrixPerspectiveOffCenterRH( matrix4x4 *pOut, float l, float r, float b, float t, float zn,float zf );// Build a perspective projection matrix. (right-handed)
	PHANTOM_API matrix4x4* 			matrixPerspectiveOffCenterLH( matrix4x4 *pOut, float l, float r, float b, float t, float zn,float zf );// Build a perspective projection matrix. (left-handed)
	PHANTOM_API matrix4x4* 			matrixOrthoRH( matrix4x4 *pOut, float w, float h, float zn, float zf );// Build an ortho projection matrix. (right-handed)
	PHANTOM_API matrix4x4* 			matrixOrthoLH( matrix4x4 *pOut, float w, float h, float zn, float zf );// Build an ortho projection matrix. (left-handed)
	PHANTOM_API matrix4x4* 			matrixOrthoOffCenterRH( matrix4x4 *pOut, float l, float r, float b, float t, float zn,float zf );// Build an ortho projection matrix. (right-handed)
	PHANTOM_API matrix4x4* 			matrixOrthoOffCenterLH( matrix4x4 *pOut, float l, float r, float b, float t, float zn,float zf );// Build an ortho projection matrix. (left-handed)
	PHANTOM_API matrix4x4* 			matrixShadow( matrix4x4 *pOut, const float4 *pLight,const Plane *pPlane );// Build a matrix which flattens geometry into a plane, as if casting a shadow from a light.
	PHANTOM_API matrix4x4* 			matrixReflect( matrix4x4 *pOut, const Plane *pPlane );// Build a matrix which reflects the coordinate system about a plane
	PHANTOM_API void				getMatrixScale(float3& outScale,const matrix4x4& matIn);
};
