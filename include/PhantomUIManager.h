...
class  PHANTOM_API WayPoints: public Base
{
public:
	WayPoints();
	virtual ~WayPoints();
...
public:
	...
	VOID					SetWayCount(INT nNewCount){m_points.resize(nNewCount);}
...
public:
	...
	float					m_fLength;
	double					m_fLastTime;
	TypeArray<WayPointInfo>	m_points;
...
public:
	...
	TypeArray<TextureRect>*	GetTextureRectPtr(int textureID);										//返回图片指针
...
protected://avatar
	...
	TypeArray<UIDialogRT*>	m_avatars;//纸娃娃新建
	TypeArray<UIDialog*>	m_dlgAvatars;//纸娃娃对话框类
	char						m_requestSortAvatar;//是否需要重新排序
protected:
	IMyUICallback*				m_callback;
	TypeArray< TextureNode2D* >		m_textureMap;
	TypeArray< FontNode2D* >		m_fontMap;
	//
...
class	 PHANTOM_API UIControlRT: public Base//UI2D模块的身体部件
{
public:
...
public:
	...
	TypeArray<char>		m_textVal;//
};
class	 PHANTOM_API UIDialogRT: public Base
{
public:
...
public:
	...
	float				m_fRot2DAngle;
	TypeArray<UIDrawMgr*>	m_drawMgrs;
	TypeArray<short>			m_nextActions;
};
...
PHANTOM_API 	UIManager*				GetDlgManager();
PHANTOM_API 	WayPoints*				CreateWayPoints();
